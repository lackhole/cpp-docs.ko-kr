...
C++에서 컴파일러는 형식을 자체적으로 선언하지 않을 경우 기본 생성자, 복사 생성자, 복사 할당 연산자 및 소멸자를 자동으로 생성합니다. 이러한 함수 라고 합니다 *특수 멤버 함수*에서 간단한 사용자 정의 형식 하 게 하 고 C++ C.의 구조와 유사 하 게 동작 즉, 만들 하 고, 복사 하 고, 추가 코딩 작업 없이 파기 수 있습니다. C++11은 언어에 이동 의미 체계를 가져오고 이동 생성자와 이동 할당 연산자를 컴파일러가 자동으로 생성할 수 있는 특수 멤버 함수 목록에 추가합니다.
