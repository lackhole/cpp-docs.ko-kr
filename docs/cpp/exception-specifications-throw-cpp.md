...
```
컴파일러에 함수가 아무 예외도 throw하지 않음을 알립니다. However, in **/std:c++14** mode this could lead to undefined behavior if the function does throw an exception. Therefore we recommend using the [noexcept](../cpp/noexcept-cpp.md) operator instead of the one above:
...
C 함수에서는 명시적 예외 사양이 허용되지 않습니다. A C function is assumed not to throw exceptions under **/EHsc**, and may throw structured exceptions under **/EHs**, **/EHa**, or **/EHac**.
...
|C++ function with `noexcept`, `noexcept(true)`, or `throw()` exception specification|아니요|아니요|예|예|
|C++ function with `noexcept(false)`, `throw(...)`, or `throw(type)` exception specification|예|예|예|예|
