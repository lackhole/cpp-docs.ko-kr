...
A template function that copies its argument might be declared **noexcept** on the condition that the object being copied is a plain old data type (POD). 이러한 함수는 다음과 같이 선언될 수 있습니다.
