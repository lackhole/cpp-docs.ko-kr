...
[__Declspec](../cpp/declspec.md) 확장 특성 [uuid](../cpp/uuid-cpp.md) 를 사용 하 여 COM 개체에 guid (globally unique identifier)를 할당할 수 있습니다. [__Uuidof](../cpp/uuidof-operator.md) 키워드는 COM 개체와 연결 된 GUID를 추출 하는 데 사용할 수 있습니다. 다른 **__declspec** 특성 [속성](../cpp/property-cpp.md)을 사용 하 여 COM 개체의 데이터 멤버에 대 한 `get` 및 `set` 메서드를 지정할 수 있습니다.

COM 집합은 전역 함수 및 클래스를 지원 하 여 `VARIANT` 및 `BSTR` 형식을 지원 하 고 스마트 포인터를 구현 하 고 `_com_raise_error`에서 throw 되는 오류 개체를 캡슐화 합니다.
