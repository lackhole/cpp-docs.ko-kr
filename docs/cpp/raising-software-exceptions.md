...
- 문제 `RaiseException` 를 검색할 때를 호출 합니다.
...
|비트|권장하는 이진 설정|설명|
|----------|--------------------------------|-----------------|
|31-30|11|이러한 두 비트는 코드의 기본 상태를 설명 합니다.  11 = 오류, 00 = 성공, 01 = 정보, 10 = 경고|
...
예외 코드를 정의한 후 예외 코드를 사용하여 예외를 발생시킬 수 있습니다. 예를 들어 다음 코드는 메모리 할당 `STATUS_INSUFFICIENT_MEM` 문제에 대 한 응답으로 예외를 발생 시킵니다.
