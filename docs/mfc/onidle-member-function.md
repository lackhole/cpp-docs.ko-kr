...
백그라운드 작업을 수행하려면 `OnIdle`을 재정의 합니다. 기본 버전은 툴바버튼과 같은 사용자 인터페이스 개체의 상태를 업데이트하고 작업 과정에서 프레임워크가 생성한 임시개체를 정리합니다. 다음 그림은 큐에 메시지가 없는 경우 메시지 루프가 `OnIdle`을  호출하는 방법을 보여줍니다.
