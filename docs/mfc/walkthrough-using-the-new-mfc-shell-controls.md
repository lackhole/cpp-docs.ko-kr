...
- Visual Studio 2017 이상 버전에서는 MFC 지원은 선택적 구성 요소입니다. 을 설치 하려면 Windows 시작 메뉴에서 Visual Studio 설치 관리자를 엽니다. 사용 하 고 선택 하는 Visual Studio의 버전을 확인 합니다 **수정** 단추입니다. 있는지 확인 합니다 **를 사용한 데스크톱 개발 C++**  타일 확인란이 선택 되어 있습니다. 아래 **선택적 구성 요소**를 확인 합니다 **MFC 지원** 단추입니다.
...
이러한 단계는 사용 중인 Visual Studio의 버전에 따라 달라 집니다. 버전 선택기 왼쪽 위에 있는이 페이지의 올바르게 설정 되어 있는지 확인 합니다.

::: moniker range="vs-2019"

### <a name="to-create-an-mfc-project-in-visual-studio-2019"></a>Visual Studio 2019에 MFC 프로젝트를 만들려면

1. 주 메뉴에서 선택 **파일** > **새로 만들기** > **프로젝트** 열려는 합니다 **새 프로젝트를 만들** 대화 상자입니다.

1. 맨 위에 있는 검색 상자에 입력 **MFC** 를 선택한 후 **MFC 앱** 결과 목록에서. 

1. **다음**을 클릭합니다. 다음 페이지에서 프로젝트의 이름을 입력 하 고 원하는 경우 프로젝트 위치를 지정 합니다.

1. 선택 된 **만들기** 프로젝트를 만들려면 단추입니다.

   이후에 **MFC 응용 프로그램 마법사** 표시는 다음 옵션을 사용 합니다.
 
   1. 선택할 **응용 프로그램 종류** 왼쪽에 있습니다. 선택한 **단일 문서로** 선택한 **문서/뷰 아키텍처 지원**합니다. 아래 **스타일 프로젝트**를 선택 **Visual Studio**, 및를 **비주얼 스타일 및 색** 목록 드롭다운 **Office 2007 (파랑 테마)**.

   1. 에 **복합 문서 지원** 창 **None**합니다.

   1. 변경 하지 않도록 합니다 **문서 템플릿 속성** 창입니다.

   1. 에 **사용자 인터페이스 기능** 창 있는지 확인 합니다 **메뉴 및 도구 모음을 사용 하 여** 옵션을 선택 합니다. 다른 옵션은 모두 그대로 둡니다.

   1. 에 **고급 기능** 창 **ActiveX 컨트롤**, **공용 컨트롤 매니페스트**, 및 **탐색창** 옵션입니다. 기타 등등 그대로 둡니다. 합니다 **탐색창** 옵션을 사용 하 여 창의 왼쪽 창을 만드는 마법사를 사용 하면 됩니다는 `CMFCShellTreeCtrl` 이미 포함 합니다.
...
### <a name="to-create-an-mfc-project-in-visual-studio-2017-or-earlier"></a>또는 이전 버전 Visual Studio 2017에서 MFC 프로젝트를 만들려면

1. 사용 된 **MFC 응용 프로그램 마법사** 새 MFC 응용 프로그램을 만드는 합니다. 마법사를 실행 하는 **파일** 메뉴 선택 **새로 만들기**를 선택한 후 **프로젝트**합니다. 합니다 **새 프로젝트** 대화 상자가 표시 됩니다.

1. 에 **새 프로젝트** 대화 상자에서 **Visual C++**  에 노드를 **프로젝트 형식** 창과 선택 **MFC**. 그런 다음 합니다 **템플릿을** 창 **MFC 응용 프로그램**합니다. 같은 프로젝트의 이름을 입력 `MFCShellControls` 누릅니다 **확인**합니다. 
...
응용 프로그램을 빌드하고 실행 하 여 만들어졌는지 확인 합니다. 응용 프로그램을 작성 하는 **빌드합니다** 메뉴 선택 **솔루션 빌드**합니다. 응용 프로그램이 성공적으로 빌드되면를 선택 하 여 응용 프로그램을 실행 **디버깅 시작** 에서 합니다 **디버그** 메뉴.
...
- 합니다 `CMFCShellListCtrl` 와 같은 다른 모드에서 파일을 볼 수 **큰 아이콘**를 **작은 아이콘**를 **목록**, 및 **세부 정보**합니다. 이 기능을 구현 하는 응용 프로그램을 업데이트 합니다. 힌트: 참조 [시각적 C++ 샘플](../overview/visual-cpp-samples.md)합니다.
