...
이 문서에서는 MFC 모듈의 상태 데이터와 및 모듈에 들어오고 나갈때의 실행흐름(응용 프로그램이 실행될 때의 코드 흐름)이 업데이트 되는 방법에 대하여 설명합니다. AFX_MANAGE_STATE 및 METHOD_PROLOGUE 매크로를 사용한 모듈 상태 전환도 설명합니다.
