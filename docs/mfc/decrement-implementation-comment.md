...
`// Implementation` 주석 아래에 선언된 맴버가 **public**, **protected** 또는 **private**으로 선언될 수 있다는 것을 [주석 예제](../mfc/an-example-of-the-comments.md)의 CStdioFile에서 확인할 수 있습니다. 이 멤버는 나중에 변경될 수 있으므로 주의해서 사용해야 합니다. 클래스 라이브러리 구현이 올바르게 작동하려면 멤버 그룹을 **public**으로 선언해야 합니다. 그러나 그렇게 선언된 멤버를 안전하게 사용할 수 있다는 의미는 아닙니다.
