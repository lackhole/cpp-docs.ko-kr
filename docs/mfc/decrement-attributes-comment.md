...
MFC 클래스 선언의 `// Attributes` 섹션은 개체의 공용 특성(또는 속성)을 포함합니다. 일반적으로 여기에 포함된 항목은 멤버 변수 또는 Get/Set 함수입니다. "Get"이나 "Set" 함수는 가상일 수도 있고 아닐 수도 있습니다. 대부분의 경우 "Get" 함수는 부작용이 없어야 하기 때문에 일반적으로 **const** 속성입니다. 이러한 멤버는 일반적으로 public입니다. protected나 private 속성은 대체로 구현 섹션에 있습니다.
