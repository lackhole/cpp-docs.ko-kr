---
title: 'TN035: 여러 리소스 파일과 헤더 파일을 사용 하 여 시각적 개체를 사용 하 여C++'
ms.date: 11/04/2016
f1_keywords:
- vc.resources
helpviewer_keywords:
...
- **Visual의 개요 C++ 관리 리소스 파일과 헤더 파일** 시각적 개체의 Resource Set Includes 명령을 하는 방법을 간략하게 설명 C++ 여러 리소스 파일과 같은 프로젝트에서 헤더 파일을 사용할 수 있습니다.
...
- **적용 되지 않은 편집 가능한 시각적 개체의 C++ 파일** 를 만드는 방법을 확인 Visual 설명 C++ 편집 및 사용자 지정 리소스를 실수로 서식을 다시 지정 하지 않습니다.

- **여러 시각적 개체에서 공유 하는 기호 관리 C++-편집 합니다. RC 파일** 여러 간에 동일한 기호를 공유 하는 방법에 설명 합니다. RC 파일 하는 방법과 중복 ID 숫자 값을 할당 하지 마세요.

- **간의 종속성을 관리 합니다. RC입니다. CPP, 및입니다. H 파일** 에 대해 설명 하는 방법을 Visual C++ 불필요 한 다시 컴파일 방지 합니다. 리소스 기호 파일에 의존 하는 CPP 파일입니다.

- **Visual C++ 에서 Set Includes 정보가** Visual 하는 방법에 대 한 기술 세부 정보를 제공 합니다. C++ 여러 (중첩 됨)의 추적 합니다. RC 파일 및 여러 헤더 파일을 # 하 여 include로 지정 된는 합니다. RC 파일입니다.

**Visual의 개요 C++ 리소스 파일 및 헤더 파일 관리**
...
기본적으로 Visual C++에서는 리소스 파일의 이름(예: MYAPP.RC)과 관계없이 해당하는 헤더 파일의 이름을 RESOURCE.H로 지정합니다. 사용 하 여는 **리소스 내용** 명령을 **뷰** 시각적 개체에 대 한 메뉴 C++의 기호 헤더 파일을 업데이트 하 여이 헤더 파일의 이름을 변경할 수 있습니다는 **SetIncludes** 대화 상자.

**읽기 전용 기호 지시문**

Visual C++에서는 지정된 .RC 파일에 대해 헤더 파일 하나만 편집하지만, 추가 읽기 전용 헤더 파일에 정의된 기호에 대한 참조를 지원합니다. 사용 하 여는 **리소스 내용** 명령을 합니다 **보기** 시각적 개체에 대 한 메뉴 C++, 읽기 전용 기호 지시문으로 임의 개수의 추가 읽기 전용 헤더 파일을 지정할 수 있습니다. "읽기 전용" 제한은 .RC 파일에 새 리소스를 추가할 경우 읽기 전용 헤더 파일에 정의된 기호를 사용할 수 있지만 리소스를 삭제할 경우 기호가 읽기 전용 헤더 파일에 여전히 정의되어 있음을 의미합니다. 읽기 전용 기호에 할당된 숫자 값은 변경할 수 없습니다.

**컴파일 시간 지시문**

Visual C++에서는 .RC 파일이 다른 .RC 파일 안에 #include로 지정되는 리소스 파일 중첩 기능도 지원됩니다. Visual C++를 사용하여 지정된 .RC 파일을 편집할 경우 #include로 지정된 파일의 리소스를 볼 수 없습니다. 하지만 .RC 파일을 컴파일할 때 #include로 지정된 파일도 컴파일됩니다. 사용 하 여는 **리소스 내용** 명령을 **보기** 시각적 개체에 대 한 메뉴 C++를 개수에 관계 없이 지정할 수 있습니다 # include로 지정 된 합니다. 컴파일 시간 지시문으로 RC 파일입니다.

될까요 시각적 개체로 읽는 경우 C++ 는 합니다. RC 파일을 #include 다른 합니다. RC 파일 *되지* 컴파일 시간 지시문으로 지정 합니다. 이 상황은 이전에 텍스트 편집기를 사용하여 수동으로 관리해 왔던 .RC 파일을 Visual C++로 가져올 때 발생할 수 있습니다. Visual C++에서 #include로 지정된 .RC 파일을 읽을 때 #include로 지정된 리소스가 부모 .RC 파일에 병합됩니다. 부모 .RC 파일을 저장할 경우 #include 문이 사실상 #include로 지정된 리소스로 대체됩니다. 이 병합이 발생 하지 않으려면 제거 해야 합니다 #include 문을 부모에서입니다. RC 파일 *이전* 시각적 개체로 읽어 C++; 시각적 개체를 사용 하 여 다음 C++, 추가 다시 동일한 #include 문을 컴파일 시간 지시문으로 합니다.

Visual C++ 에서 저장을 합니다. RC 파일은 위의 세 가지 Set Includes 정보 (기호 헤더 파일, 읽기 전용 기호 지시문 및 컴파일 시간 지시문)에서 #include 지시문 *고* TEXTINCLUDE 리소스에서입니다. TEXTINCLUDE 리소스는 구현 세부 정보를 처리 하기 위해 일반적으로 필요 하지 않은 설명 [Visual C++ 에서 Set Includes 정보가](#_mfcnotes_tn035_set_includes)합니다.
...
MYAPP 합니다. RC 시각적 개체 사용을 편집 하는 응용 프로그램 리소스 파일 C++입니다.
...
응용 프로그램 마법사가 만든 응용 프로그램에는 두 개의 헤더 파일이 포함 됩니다. 리소스입니다. H와 AFXRES 합니다. 8. RESOURCE.H는 애플리케이션마다 다릅니다. 다음의 경우에는 추가 읽기 전용 헤더 파일만 포함하면 됩니다.
...
마법사가 만든 RES\MYAPP 합니다. RC2 파일은 리소스를 포함 하는 파일의 예 *되지* 시각적 개체로 읽으려는 실수로 C++ 및 서식 정보가 손실 될 수 있는 다시 다음 씁니다. 이러한 경우가 발생하지 않도록 하기 위해 RES\MYAPP.RC2 파일의 시작 부분에 다음 줄을 추가합니다.
...
Visual 하는 경우 C++ 컴파일 합니다. 또한 RC 파일인 `APSTUDIO_INVOKED` 뿐만 `RC_INVOKED`합니다. 응용 프로그램 마법사가 만든 파일 구조가 손상되고 Visual C++에서 위의 #error 줄을 읽고 심각한 오류로 보고한 후 .RC 파일의 읽기를 중단합니다.

**여러 시각적 개체에서 공유 하는 기호 관리 C++-편집 합니다. RC 파일**
...
Visual C++는 새 .RC 파일을 만들 때 최저 유효 값보다 조금 높은 값부터 시작합니다. 또한 애플리케이션 마법사는 이러한 값을 MFC 애플리케이션에 보다 적합한 형태로 초기화합니다. ID 값 범위에 대 한 자세한 내용은 참조 하세요. [Technical Note 20](../mfc/tn020-id-naming-and-numbering-conventions.md)합니다.

이제 Visual 동일한 프로젝트에도 새 리소스 파일을 만들 때마다 C++ 동일한 정의 `_APS_NEXT_` 값입니다. 다시 말해서 두 .RC 파일에 여러 대화 상자를 추가하는 경우 동일한 #define 값이 서로 다른 대화 상자에 할당될 가능성이 높습니다. 예를 들면 첫 번째 .RC 파일의 IDD_MY_DLG1에는 두 번째 .RC 파일의 IDD_MY_DLG2와 동일한 101이라는 번호가 할당될 수 있습니다.
...
에 저장 된 각 위의 세 가지 Set Includes 정보 유형의 합니다. 두 가지 형태로 RC 파일: (1) #include 또는 다른 지시문 해석할 수 있는 (2)로 해석할 수 있는 특수 TEXTINCLUDE 리소스 및 리소스 컴파일러에 의해 시각적 개체에 의해서만 C++입니다.

TEXTINCLUDE 리소스의 목적은 쉽게 표시할 수 있는 형태로 Set Include 정보를 안전 하 게 저장 하는 것 시각적 개체의 C++의 **Set Includes** 대화 상자. TEXTINCLUDE는 한 *리소스 종류* 시각적 개체에서 정의한 C++합니다. Visual C++에서는 리소스 ID 1, 2 및 3이 지정된 세 가지 특정 TEXTINCLUDE 리소스를 인식합니다.
