...
|[CDaoFieldExchange::SetFieldType](#setfieldtype)|다음에를 `SetFieldType`호출할 때까지 DFX 함수에 대 한 모든 후속 호출로 표시 되는 레코드 집합 데이터 멤버 (열 또는 매개 변수)의 유형을 지정 합니다.|
...
개체 `CDaoFieldExchange` 는 DAO 레코드 필드 교환이 이루어지는 데 필요한 컨텍스트 정보를 제공 합니다. `CDaoFieldExchange`개체는 바인딩 매개 변수 및 필드 데이터 멤버를 비롯 한 다양 한 작업을 지원 하 고 현재 레코드의 필드에 다양 한 플래그를 설정 합니다. DFX 작업은의 `CDaoFieldExchange` **enum** **FieldType** 에서 정의 된 형식의 레코드 집합 클래스 데이터 멤버에 대해 수행 됩니다. 가능한 **FieldType** 값은 다음과 같습니다.
...
사용자 고유의 DFX 함수를 작성 하는 경우 `IsValidOperation` 함수 시작 부분에서를 호출 하 여 특정 필드 데이터 멤버 형식 `CDaoFieldExchange::outputColumn` (또는 `CDaoFieldExchange::param`)에 대해 현재 작업을 수행할 수 있는지 여부를 확인 합니다.
...
개체 `CDaoFieldExchange` 는 레코드 집합에 대 한 다양 한 DFX 작업의 컨텍스트를 제공 합니다.
...
의 `m_nOperation` 가능한 값은 다음과 같습니다.
...
`CDaoRecordset` 클래스 `SetFieldType` 의`DoFieldExchange` 재정의에서를 호출 합니다.
...
*nFieldType*<br/>
에`CDaoFieldExchange`선언 된 **열거형 FieldType**의 값으로, 다음 중 하나일 수 있습니다.
...
일반적으로 클래스 마법사는이 호출을 작성 합니다. 사용자 고유의 함수를 작성 하 고 마법사를 사용 하 여 `DoFieldExchange` 함수를 작성 하는 경우 필드 맵 외부의 함수에 대 한 호출을 추가 합니다. 마법사를 사용 하지 않는 경우에는 필드 맵이 표시 되지 않습니다. 이 호출은 클래스의 각 필드 데이터 멤버에 대 한 DFX 함수 호출을 앞에,로 `CDaoFieldExchange::outputColumn`필드 형식을 식별 합니다.

레코드 집합 클래스를 매개 변수화 하는 경우 모든 매개 변수 데이터 멤버 (필드 맵 외부)에 대해 DFX 호출을 추가 하 고를 `SetFieldType`호출 하 여 이러한 호출 앞에와 야 합니다. 값 `CDaoFieldExchange::param`을 전달 합니다. 대신 [CDaoQueryDef](../../mfc/reference/cdaoquerydef-class.md) 를 사용 하 고 매개 변수 값을 설정할 수 있습니다.

일반적으로 필드 데이터 멤버 또는 매개 변수 데이터 멤버와 연결 된 DFX 함수 호출의 각 그룹 앞에는를 `SetFieldType`호출 해야 합니다. `SetFieldType` 각`SetFieldType` 호출의 nFieldType 매개 변수는 호출을 따르는 DFX 함수 호출로 표시 되는 데이터 멤버의 형식을 식별 합니다.
