...
[CATCH](../../mfc/reference/exception-processing.md#catch) 식의 범위 내에서 예외 개체에 액세스할 수 있습니다. [AfxThrowDaoException global](../../mfc/reference/exception-processing.md#afxthrowdaoexception) 함수를 사용하여 사용자 고유의 코드에서 `CDaoException` 개체를 throw 할 수도 있습니다.

MFC에서 모든 DAO 오류는 형식의 `CDaoException`예외로 표시 됩니다. 이 형식의 예외를 catch 할 때 멤버 함수를 사용 `CDaoException` 하 여 데이터베이스 엔진의 Errors 컬렉션에 저장 된 DAO 오류 개체에서 정보를 검색할 수 있습니다. 오류가 발생할 때마다 하나 이상의 오류 개체가 Errors 컬렉션에 배치 됩니다. 일반적으로 컬렉션은 하나의 오류 개체만 포함 합니다. ODBC 데이터 원본을 사용 하는 경우에는 여러 오류 개체를 가져올 가능성이 높습니다. 다른 DAO 작업에서 오류가 생성 되 면 Errors 컬렉션이 지워지고 새 error 개체가 Errors 컬렉션에 배치 됩니다. 오류를 생성 하지 않는 DAO 작업은 Errors 컬렉션에 영향을 주지 않습니다.

DAO 오류 코드는 DAOERR 파일을 참조 하세요. 넣기. 관련 정보는 DAO 도움말의 "잡을 수 있는 데이터 액세스 오류" 항목을 참조 하십시오.

일반적인 예외 처리 또는 개체 정보 `CDaoException` 에 대 한 자세한 내용은 [예외 처리 (MFC)](../../mfc/exception-handling-in-mfc.md) 및 [예외 문서를 참조 하세요. 데이터베이스 예외](../../mfc/exceptions-database-exceptions.md). 두 번째 문서에는 DAO의 예외 처리를 보여 주는 예제 코드가 포함 되어 있습니다.
...
일반적으로 프레임 워크는 예외를 throw 하는 예외 개체를 만듭니다. 예외 개체를 명시적으로 생성 해야 하는 경우는 거의 없습니다. 사용자의 코드에서를 `CDaoException` throw 하려면 전역 함수 [AfxThrowDaoException](../../mfc/reference/exception-processing.md#afxthrowdaoexception)를 호출 합니다.
...
1. [M_pErrorInfo](#m_perrorinfo) 데이터 멤버에서 반환 하 `GetErrorInfo` 는 현재 [CDaoErrorInfo](../../mfc/reference/cdaoerrorinfo-structure.md) 구조체를 검사 합니다. 해당 멤버는 DAO 오류에 대 한 정보를 제공 합니다.
...
MFC DAO 클래스의 오류를 처리 하 [는 방법에 대 한 자세한 내용은 예외: 데이터베이스 예외](../../mfc/exceptions-database-exceptions.md).
...
`GetErrorInfo`예외 개체의 `m_pErrorInfo` 데이터 멤버에 정보를 저장 합니다. 반환 되는 정보에 대 한 간략 한 설명은 [m_pErrorInfo](#m_perrorinfo)를 참조 하세요. `CDaoException` MFC`m_pErrorInfo` 에서 throw 된 형식의 예외를 catch 하는 경우 멤버가 이미 입력 됩니다. DAO를 직접 호출 하도록 선택 하는 경우 예외 개체의 `GetErrorInfo` 멤버 함수를 직접 호출 하 여 채워야 `m_pErrorInfo`합니다. 자세한 설명은 [CDaoErrorInfo](../../mfc/reference/cdaoerrorinfo-structure.md) 구조체를 참조 하세요.

DAO 예외 및 예제 코드 [에 대 한 자세한 내용은 예외: 데이터베이스 예외](../../mfc/exceptions-database-exceptions.md).
...
- NO_AFX_DAO_ERROR 가장 최근 작업으로 인해 MFC 확장 오류가 발생 하지 않았습니다. 그러나이 작업은 DAO 또는 OLE에서 다른 오류를 생성 했을 수 있으므로 [m_pErrorInfo](#m_perrorinfo) 및 [m_scode](#m_scode)를 확인 해야 합니다.
...
- AFX_DAO_ERROR_DFX_BIND DFX (DAO 레코드 필드 교환) 함수 호출에 사용 된 주소가 없거나 잘못 되었습니다 .이 주소는 데이터를 바인딩하는 데 사용 되지 않았습니다. DFX 호출에 잘못 된 주소가 전달 되었거나 DFX 작업 간에 주소가 잘못 되었을 수 있습니다.

- AFX_DAO_ERROR_OBJECT_NOT_OPEN 열을 기반으로 하는 레코드 집합 또는 열림 상태가 아닌 테이블 정의 개체를 열려고 했습니다.

##  <a name="m_perrorinfo"></a>  CDaoException::m_pErrorInfo

[Geterrorinfo](#geterrorinfo)를 호출 하 `CDaoErrorInfo` 여 마지막으로 검색 한 DAO error 개체에 대 한 정보를 제공 하는 구조체에 대 한 포인터를 포함 합니다.
...
|`m_strDescription`|Description|오류와 관련 된 설명이 포함 된 문자열입니다.|
...
이는 OLE 코드입니다. 거의 모든 경우에 다른 `CDaoException` 데이터 멤버에서 더 구체적인 MFC 또는 DAO 오류 정보를 사용할 수 있기 때문에이 값을 사용 해야 하는 경우는 거의 없습니다.
