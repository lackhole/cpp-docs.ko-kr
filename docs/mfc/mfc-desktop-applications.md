...
MFC(Microsoft Foundation Class) 라이브러리는 많은 Win32 및 COM API에 대한 개체 지향 래퍼를 제공합니다. 간단한 데스크톱 응용 프로그램을 만드는 데도 사용되지만, 여러 컨트롤 간의 매우 복잡한 사용자 인터페이스를 개발해야 하는 경우에도 매우 유용합니다. MFC를 사용하여 Office 스타일의 사용자 인터페이스를 제공하는 응용 프로그램을 만들 수 있습니다. Windows 플랫폼 자체에 대 한 설명서는 [windows 설명서](/windows/index)를 참조 하십시오. MFC 없이에서 C++ windows 응용 프로그램을 빌드하는 방법에 대 한 자세한 내용은 [Win32 API를 사용 하 여 데스크톱 windows 앱 빌드](/windows/win32/index)를 참조 하세요.
