...
ms.locfileid: "69511178"
---
# <a name="tn014-custom-controls"></a>TN014: 사용자 지정 컨트롤

이 노트는 사용자 지정과 자체 그리기 컨트롤에 대한 MFC 지원을 설명합니다. 또한 동적 서브클래싱에 대해 설명 하 고 [CWnd](../mfc/reference/cwnd-class.md) 개체와 `HWND`의 관계를 설명 합니다.
...
MFC는 표준 소유자 그리기 메시지에 대 `CWnd` 한 기본 구현 (및 [CMenu](../mfc/reference/cmenu-class.md) 클래스)을 제공 합니다. 이러한 기본 구현은 소유자 그리기 매개 변수를 디코딩하고 소유자 그리기 메시지를 컨트롤 또는 메뉴에 위임합니다. 자체 그리기라고 부르는 이유는 그리기 코드가 소유자 창이 아닌 컨트롤 또는 메뉴의 클래스에 있기 때문입니다.
...
소유자 그리기 구조 ([drawitemstruct](/windows/win32/api/winuser/ns-winuser-drawitemstruct), [MEASUREITEMSTRUCT](/windows/win32/api/winuser/ns-winuser-measureitemstruct), [compareitemstruct](/windows/win32/api/winuser/ns-winuser-compareitemstruct)및 [deleteitemstruct](/windows/win32/api/winuser/ns-winuser-deleteitemstruct))에 대 한 자세한 `CWnd::OnDrawItem` `CWnd::OnCompareItem`내용은, `CWnd::OnMeasureItem`, 및 에대한MFC설명서를참조하십시오.`CWnd::OnDeleteItem` 각각.
...
LBS_SORT 또는 CBS_SORT 스타일을 사용 하는 자체 그리기 목록 상자 및 콤보 상자의 경우 `OnCompareItem` 메서드를 재정의 해야 합니다.
...
서브클래싱은 창의을 <xref:System.Windows.Forms.Control.WndProc%2A> 사용자 지정 `WndProc` 된로 바꾸고 기본 기능에 대해 이전 `WndProc` 을 호출 하는 Windows 용어입니다.

이 용어를 C++ 클래스 파생과 혼동해서는 안됩니다. C++ 설명을 위해 *기본 클래스* 및 *파생 클래스* 라는 용어는 Windows 개체 모델의 *슈퍼* 클래스 및 *하위* 클래스와 유사 합니다. MFC의 C++ 파생과 Windows의 서브클래싱은 기능적으로 비슷하지만 C++의 경우 동적 서브클래싱이 지원되지 않습니다.
...
- 이 애플리케이션은 `CWnd`를 기존 `HWND`에 연결합니다. 기존 창의 동작은 수정되지 않습니다. 이는 위임의 사례 이며 [CWnd:: Attach](../mfc/reference/cwnd-class.md#attach) 를 호출 하 여 `HWND` `CWnd` 개체에 대 한 기존 별칭을 지정할 수 있습니다.
