...
1. 프로그램이 예정된 내용을 수행하는 것처럼 보입니다. 첫 번째 숫자를 사용하고 두 번째로 나눕니다. `cout` 줄에서 `result` 변수를 마우스로 가리키거나 **자동** 창에서 `result`를 살펴봅니다. 올바르지 않은 것 같은 “inf”로 값이 등록된 것을 확인하게 됩니다. 해당 값을 수정하세요. `cout` 줄은 `result`에 저장된 모든 값을 출력하므로 **F10** 키를 사용하여 추가로 한 줄을 실행하면 콘솔 창에서는 다음을 표시합니다.
