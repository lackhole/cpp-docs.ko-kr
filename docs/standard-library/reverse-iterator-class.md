...
반복기의 범위는 [*first*, *last*)입니다. 여기서 왼쪽 대괄호는 *첫 번째* 를 포함 하는 것을 나타내고 오른쪽의 괄호는 *마지막* 자체를 제외 하 고는 요소를 포함 하는 것을 나타냅니다. 같은 요소는 역방향 시퀀스 [ **rev** - *first*, **rev** - *last*)에 포함 되어 있으므로 *마지막* 이 시퀀스의 하나 이전 끝 요소 이면 첫 번째 요소 **rev** - 역방향 시퀀스의 첫 번째는 \*(*last* -1)를 가리킵니다. 모든 역방향 반복기를 기본 역방향과 연결하는 ID는 다음과 같습니다.
...
|[operator->](#op-arrow)|`reverse_iterator`가 주소 지정하는 요소로 포인터를 반환합니다.|
...
`reference`를 선언 하 고 사용 하는 방법에 대 한 예제는 [reverse_iterator:: operator&#91; ](#op_at) 또는 [reverse_iterator:: operator *](#op_star) 를 참조 하세요.
