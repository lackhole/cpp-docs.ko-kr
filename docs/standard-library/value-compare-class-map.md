...
맵에 포함 된 전체 요소 `value_compare` `value_types` 중에서 제공 하는 비교 기준은 보조 클래스 생성에의 한 개별 요소의 키 간 비교에서 발생 합니다. 멤버 함수 연산자는에서 `comp` `value_compare` 제공 하는 함수 `key_compare` 개체에 저장 된 형식의 개체를 사용 하 여 두 요소의 정렬 키 구성 요소를 비교 합니다.
