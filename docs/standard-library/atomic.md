...
**C++11**: 신호 처리기에서 또는 `obj` `atomic_is_lock_free(x)` 가 true 인 경우 `obj.is_lock_free()` 개체에 대해 원자 단위 작업을 수행할 수 있습니다.
...
|[atomic_flag 구조체](../standard-library/atomic-flag-structure.md)|**Bool** 플래그를 원자 단위로 설정 하 고 지우는 개체를 설명 합니다.|
