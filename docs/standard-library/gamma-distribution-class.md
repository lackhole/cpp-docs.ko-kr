...
감마 분포에 따라 분포 된 경우 클래스 템플릿은 사용자 지정 부동 소수점 형식의 값을 생성 하는 분포를 설명 하 고, 아무것도 제공 되지 않은 경우 **double** 형식으로 지정 합니다. 다음 테이블은 개별 멤버에 대한 문서와 연결되어 있습니다.
