...
헤더는 **wchar_t**, **bool**, **char**, **signed char**, **unsigned char**, **short**, **unsigned short**, **int**, **unsigned int**, **long**형식에 대 한 명시적 특수화를 정의 합니다. **unsigned long**, **float**, **double**, **long double**, **long long**, **unsigned long long**, **char16_t**및 **char32_t**입니다. 이러한 명시적 특수화의 경우 [numeric_limits:: is_specialized](#is_specialized) 멤버는 **true**이며 모든 관련 멤버에 의미 있는 값이 있습니다. 프로그램에서 추가 명시적 특수화를 제공할 수 있습니다. 클래스의 멤버 함수 대부분은 **float**의 가능한 구현을 설명 하거나 테스트 합니다.
...
함수는 형식에 대 한 최소값을 반환 합니다 .이 값은 [has_denorm](#has_denorm) 가 `denorm_present` 같지 않은 경우 [min](#min) 과 동일 합니다.
...
형식에 대해 가장 음수 값을 반환 합니다 (일반적으로 정수 형식 및 부동 소수점 형식에 대 한 `-max()` `min()`). @No__t_0 **true**이면 반환 값은 의미가 있습니다.
...
**Float**형식에 대 한 최대 유한 값은 **INT** 형식 및 FLT_MAX에 대 한 INT_MAX입니다. 반환 값은 [is_bounded](#is_bounded)가 **true**인 경우 의미가 있습니다.
...
정규화 된 최소 값은 **float**형식에 대 한 **INT** 및 FLT_MIN 형식에 대 한 INT_MIN입니다. 반환 값은 [is_bounded](#is_bounded) 가 **true** 이거나 [is_signed](#is_signed) 가 **false**인 경우에 의미가 있습니다.
