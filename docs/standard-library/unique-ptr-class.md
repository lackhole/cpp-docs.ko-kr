...
|`operator*`|멤버 함수는 `*stored_ptr`를 반환합니다.|
|[operator=](#unique_ptr_operator_eq)|`unique_ptr` 또는 `pointer-type`의 값을 현재 `unique_ptr`에 할당합니다.|
...
### <a name="unique_ptr_operator_eq"></a> 연산자 =
...
*ptr*\
에 할당할 리소스에 대 한 포인터를 `unique_ptr`입니다.
