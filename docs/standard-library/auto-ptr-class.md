...
에 `Other` 대 한 포인터를에 대 `Type`한 포인터로 암시적으로 변환할 수 있는 경우 템플릿 생성자는 두 번째 생성자와 동일 하 게 동작 합니다.
...
포인터가 소유 하는 형식의 `Type` 개체에 대 한 참조입니다.
...
을 소유 하 `auto_ptr` 는 개체의 멤버입니다.

#### <a name="remarks"></a>설명

선택 연산자는 [get](#get)`( )`을 반환합니다. 따라서 *ap*-> **member** 식은 ( *ap*. **get**( ) )-> **member**와 동일하게 동작합니다. 여기서 *ap*는 `auto_ptr`\< **Type**> 클래스의 개체입니다. 따라서 저장 된 포인터는 null이 아니어야 하며 `Type` `member` 멤버를 포함 하는 클래스, 구조체 또는 공용 구조체 형식 이어야 합니다.

#### <a name="example"></a>예제

멤버 함수를 사용 하는 방법에 대 한 예제는 [auto_ptr](#auto_ptr)를 참조 하세요.

### <a name="op_auto_ptr_lt_other_gt"></a>auto_ptr&lt;연산자&gt;
...
### <a name="op_auto_ptr_ref_lt_other_gt"></a>auto_ptr_ref&lt;연산자&gt;
...
멤버 함수는 저장 된 포인터 `delete myptr`값 `myptr` 이 함수 호출의 결과로 변경 되는 경우에만 식을 계산 합니다. 그런 다음 저장된 포인터를 `ptr`로 바꿉니다.
...
*ptr*\
저장 된 포인터 `myptr`를 바꾸도록 지정 된 포인터입니다.
