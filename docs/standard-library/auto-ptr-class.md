...
클래스 템플릿은 할당 된 개체에 대 한 `auto_ptr` 이라는 스마트 포인터를 설명 합니다. 포인터는 null 이거나 **new**로 할당 된 개체를 지정 해야 합니다. 저장된 값이 다른 개체에 할당되면 `auto_ptr`이 소유권을 전송합니다. Null 포인터를 사용 하 여 전송 후 저장 된 값을 대체 합니다. @No__t_0에 대 한 소멸자는 할당 된 개체를 삭제 합니다. `auto_ptr<Type>`은 예외가 발생하는 경우에도 제어가 블록을 나갈 때 할당된 개체가 자동으로 삭제되도록 합니다. 동일한 개체를 소유하는 두 개의 `auto_ptr<Type>` 개체를 생성하면 안 됩니다.
...
첫 번째 생성자는 `myptr`에 *ptr* 을 저장 하 고 할당 된 개체에 저장 된 포인터를 저장 합니다. 두 번째 생성자 *는 right를*저장 하 여 *오른쪽*에 저장 된 포인터의 소유권을 전송 합니다. `myptr`에서 [릴리스](#release).

세 번째 생성자는 `right` 저장 한다는 점을 제외 하 고 두 번째 생성자와 동일 하 게 동작 합니다. `ref` `myptr`에서 `release` `ref`은 `right`에 저장 된 참조입니다.

@No__t_0에 대 한 포인터를 `Type` 포인터로 암시적으로 변환할 수 있는 경우 템플릿 생성자는 두 번째 생성자와 동일 하 게 동작 합니다.
...
할당은 식 `delete myptr`를 평가 하지만 저장 된 포인터가 `myptr` 할당의 결과로 변경 되는 경우에만 적용 됩니다. 그런 다음 *right*를 저장하여 *오른쪽*에 저장된 포인터의 소유권을 전송합니다. `myptr`에서 [릴리스](#release). 함수는 __\*this__를 반환합니다.
...
포인터가 소유 하는 `Type` 형식의 개체에 대 한 참조입니다.
...
@No__t_0 소유 하는 개체의 멤버입니다.

#### <a name="remarks"></a>주의

선택 연산자는 [get](#get)`( )`을 반환합니다. 따라서 *ap*-> **member** 식은 ( *ap*. **get**( ) )-> **member**와 동일하게 동작합니다. 여기서 *ap*는 `auto_ptr`\< **Type**> 클래스의 개체입니다. 따라서 저장 된 포인터는 null이 아니어야 하며 `Type`은 `member` 멤버가 있는 클래스, 구조체 또는 공용 구조체 형식 이어야 합니다.

#### <a name="example"></a>예제

멤버 함수를 사용 하는 방법에 대 한 예제는 [auto_ptr](#auto_ptr)를 참조 하세요.

### <a name="op_auto_ptr_lt_other_gt"></a>연산자 auto_ptr &lt;Other &gt;
...
### <a name="op_auto_ptr_ref_lt_other_gt"></a>연산자 auto_ptr_ref &lt;Other &gt;
...
멤버 함수는 `delete myptr` 식을 계산 하지만 저장 된 포인터 값 `myptr` 함수 호출의 결과로 변경 되는 경우에만 적용 됩니다. 그런 다음 저장된 포인터를 `ptr`로 바꿉니다.
...
*ptr* \
저장 된 포인터 `myptr`를 대체 하기 위해 지정 된 포인터입니다.
