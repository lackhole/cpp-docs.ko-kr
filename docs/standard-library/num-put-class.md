...
ms.locfileid: "72687635"
---
# <a name="num_put-class"></a>num_put 클래스

@No__t_0 형식의 시퀀스로 숫자 값의 변환을 제어 하는 로캘 패싯으로 사용할 수 있는 개체를 설명 하는 클래스 템플릿입니다.
...
정수 출력 필드는 일련의 **char** 요소를 파일에 생성 하기 위해 인쇄 함수에서 사용 하는 것과 동일한 규칙에 의해 생성 됩니다. 이러한 각 char 요소는 단순 하 고 일대일 매핑으로 `CharType` 형식의 동등한 요소로 매핑됩니다. Print 함수는 공백이 나 숫자 0으로 필드를 채웁니다. 그러나 `do_put` 대신 `fill`를 사용 합니다. 동일한 인쇄 변환 사양은 다음과 같이 결정됩니다.
...
- 그렇지 않으면 변환 사양은 **lg**입니다. **iosbase**. **플래그**  & **ios_base:: 대문자** 는 0이 아닌 `g` `G`로 바뀝니다.
...
는 변환 사양의 `l` 한정자가 `L`으로 바뀌는 것을 제외 하 고 세 번째와 동일 하 게 동작 합니다.
...
부울 출력 필드는 두 가지 형식 중 하나를 사용합니다. [Boolalpha](../standard-library/ios-functions.md#boolalpha) 이 **false**인 `iosbase.flags & ios_base::` 경우 멤버 함수는 일반적으로 0 ( **false**의 경우) 또는 1 ( **true**의 경우)의 생성 된 시퀀스를 생성 하는 `do_put(_Next, _Iosbase, _Fill, (long)val)` 반환 합니다. 그렇지 않으면 생성 된 시퀀스는 *fac*입니다. [falsename](../standard-library/numpunct-class.md#falsename) ( **false**의 경우) 또는 *fac*. [truename](../standard-library/numpunct-class.md#truename) ( **true**의 경우).
...
숫자를 지정 된 로캘에 대해 서식이 지정 된 숫자를 나타내는 `CharType`s 시퀀스로 변환 합니다.
