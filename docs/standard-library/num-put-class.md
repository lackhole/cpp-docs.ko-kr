...
변환 사양의 한정자 `l` 가로 `L`대체 되는 점을 제외 하 고 세 번째와 동일한 동작을 수행 합니다.
...
부울 출력 필드는 두 가지 형식 중 하나를 사용합니다. `iosbase.flags & ios_base::` [Boolalpha](../standard-library/ios-functions.md#boolalpha) 이 **false**인 경우 멤버 함수는 일반적 `do_put(_Next, _Iosbase, _Fill, (long)val)`으로 0 ( **false**의 경우) 또는 1 ( **true**의 경우)의 생성 된 시퀀스를 생성 하는를 반환 합니다. 그렇지 않으면 생성 된 시퀀스는 *fac*입니다. [falsename](../standard-library/numpunct-class.md#falsename) ( **false**의 경우) 또는 *fac*. [truename](../standard-library/numpunct-class.md#truename) ( **true**의 경우).
...
숫자를 지정 된 로캘에 대해 서식이 `CharType`지정 된 숫자를 나타내는의 시퀀스로 변환 합니다.
