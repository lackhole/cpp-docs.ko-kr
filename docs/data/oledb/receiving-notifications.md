...
`IRowsetNotify` 에 여러 번 호출할 수 있는 세 가지 메서드가 있습니다. 이들 메서드 중 하나에만 응답하려는 경우, 응답하지 않을 메서드에 대해 `E_NOTIMPL`을 반환하는 [IRowsetNotifyImpl](../../data/oledb/irowsetnotifyimpl-class.md) 클래스를 사용할 수 있습니다.

행 집합을 만들 경우, 알림 설정에 필요한 `IConnectionPointContainer`를 지원하는 반환된 행 집합 개체가 필요하다는 점을 공급자에게 알려야 합니다.

다음 코드에서는 ATL 개체에서 행 집합을 열고 `AtlAdvise` 함수를 사용하여 알림 싱크를 설정하는 방법을 보여 줍니다. `AtlAdvise` 호출할 때 사용 되는 쿠키를 반환 `AtlUnadvise`합니다.
