...
기본적으로 Visual C++ 링커는 실행 수준이 `asInvoker`인 응용 프로그램의 매니페스트에 UAC 조각을 포함 합니다. 관리 권한이 있어야 애플리케이션이 제대로 실행되는 경우(예: 레지스트리의 HKLM 노드를 수정하거나 Windows 디렉터리와 같은 디스크의 보호된 영역에 쓰는 경우) 애플리케이션을 수정해야 합니다.
