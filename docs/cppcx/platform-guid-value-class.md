...
`Platform::Guid`에는 [platform:: Object 클래스](../cppcx/platform-object-class.md)에서 파생 된 `Equals()`, `GetHashCode()`및 `ToString()` 메서드와 [Platform:: Type 클래스](../cppcx/platform-type-class.md)에서 파생 된 `GetTypeCode()` 메서드가 있습니다. 또한 `Platform::Guid`에는 다음과 같은 멤버도 있습니다.

|멤버|설명|
|------------|-----------------|
|[Guid](#ctor)|`Platform::Guid`의 새 인스턴스를 초기화합니다.|
|[operator==](#operator-equality)|Equals 연산자입니다.|
...
*m*<br/>
[GUID 구조](/previous-versions/cc317743(v%3dmsdn.10))를 형성 하는의 `GUID`입니다.
...
*Guid1* 가 *guid2*앞에 정렬 되 면 True입니다. 사전적는 4 32 비트 부호 없는 값의 배열인 것 처럼 각 `Platform::Guid`를 처리 한 후에 정렬 됩니다. 이는 SQL Server 또는 .NET Framework에서 사용 되는 순서가 아니라 문자열 표현에 따라 사전순으로 정렬 하는 것과 동일 하지 않습니다.

이 연산자는 `Guid` 개체를 C++ 표준 라이브러리에서 보다 쉽게 사용할 수 있도록 제공 됩니다.

## <a name="operator-call"></a>Guid:: operator () 연산자

`Platform::Guid`를 [GUID 구조체로](/previous-versions/cc317743(v%3dmsdn.10))암시적으로 변환 합니다.
