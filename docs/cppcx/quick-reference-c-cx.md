...
ms.locfileid: "70740913"
---
# <a name="quick-reference-ccx"></a>빠른 참조(C++/CX)

Windows 런타임는 신뢰할 수 있는 운영 체제 환경 에서만 실행 되 고, 권한 있는 함수, 데이터 형식 및 장치를 사용 하며, Microsoft Store를 통해 배포 되는 UWP (유니버설 Windows 플랫폼) 앱을 지원 합니다. /Cx C++는 Windows 런타임에 대 한 앱 작성을 간소화 합니다. 이 문서는 빠른 참조입니다. 전체 설명서는 [형식 시스템](../cppcx/type-system-c-cx.md)을 참조 하세요.
...
|개념|표준 C++|C++/CX|설명|
|-------------|--------------------|------------------------------------------------------------------|-------------|
|기본 형식|C++ 기본 형식|C++Windows 런타임에 정의 된 기본 형식을 구현 하는/CX 기본 형식입니다.|네임 `default` 스페이스에 C++는/cx 기본 제공 기본 형식이 포함 되어 있습니다. 컴파일러는/Cx 기본 C++형식을 표준 C++ 형식에 암시적으로 매핑합니다.<br /><br /> 네임 `Platform` 스페이스의 패밀리에는 기본 Windows 런타임 형식을 구현 하는 형식이 포함 되어 있습니다.|
