...
개체가 나타내는 두 문자열 값에서 해당 문자의 숫자 값을 계산 하 여 두 `String` 개체를 비교 하는 정적 메서드입니다.
...
@No__t_0 유니코드 문자 배열의 시작 부분에 대 한 포인터입니다 (`char16`는 `wchar_t`에 대 한 typedef).

### <a name="remarks"></a>주의

이 메서드를 사용하여 `Platform::String^`에서 `wchar_t*`로 변환합니다. `String` 개체가 범위를 벗어나는 경우 데이터 포인터의 유효성이 더 이상 보장되지 않습니다. 원래 `String` 개체의 수명 이상으로 데이터를 저장 하려면 [wcscpy_s](../c-runtime-library/reference/strcpy-s-wcscpy-s-mbscpy-s.md) 를 사용 하 여 사용자가 할당 한 메모리에 배열을 복사 합니다.

## <a name="dispose"></a>문자열::D ispose 메서드
...
`str` 현재 개체와 같으면 **true** 이 고, 그렇지 않으면입니다. 그렇지 않으면 **false**입니다.
...
현재 `String` 개체가 **null** 이거나 빈 문자열 (L "") 이면 **true** 입니다. 그렇지 않으면 **false**입니다.
...
현재 `String` 개체가 빠름 이면 **true** 이 고, 그렇지 않으면입니다. 그렇지 않으면 **false**입니다.
...
*str1* 가 *str2*와 같으면 **true** 입니다. 그렇지 않으면 **false**입니다.
...
`str1`의 내용이 `str2`와 같으면 **true** 이 고, 그렇지 않으면 **false**입니다.
...
한 `String` 개체의 값이 두 번째 `String` 개체의 값 보다 큰지 여부를 나타냅니다.
...
`str1` 값이 `str2` 값 보다 크면 **true** 입니다. 그렇지 않으면 **false**입니다.
...
한 `String` 개체의 값이 두 번째 `String` 개체의 값 보다 크거나 같은지 여부를 나타냅니다.
...
`str1` 값이 `str2`의 값 보다 크거나 같으면 **true** 입니다. 그렇지 않으면 **false**입니다.

## <a name="operator-inequality"></a>String:: operator! =

지정 된 두 `String` 개체의 값이 다른 지 여부를 나타냅니다.
...
`str1` `str2`와 같지 않으면 **true** 입니다. 그렇지 않으면 **false**입니다.

## <a name="operator-less-than"></a>String:: operator &lt;

한 `String` 개체의 값이 두 번째 `String` 개체의 값 보다 적은지 여부를 나타냅니다.
...
*str1* 의 값이 *str2*값 보다 작은 경우 **true** 입니다. 그렇지 않으면 **false**입니다.

## <a name="ctor"></a>String:: String 생성자

입력 문자열 데이터의 복사본을 사용 하 여 `String` 클래스의 새 인스턴스를 초기화 합니다.
...
현재 문자열과 같은 값을 갖는 `String` 개체를 반환 합니다.
