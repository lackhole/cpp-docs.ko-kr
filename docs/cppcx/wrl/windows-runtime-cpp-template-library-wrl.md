...
[ComPtr](comptr-class.md) 은 템플릿 매개 변수를 통해 지정된 인터페이스를 나타내는 *스마트 포인터* 형식입니다. `ComPtr` 을 사용하여 인터페이스에서 파생된 개체의 멤버를 액세스할 수 있는 변수를 정의합니다. `ComPtr`은 기본 인터페이스 포인터의 참조 횟수를 자동으로 유지 관리 하 고 참조 횟수가 0이 되 면 인터페이스를 해제 합니다.
...
[FtmBase](ftmbase-class.md) 는 자유 스레드된 마샬러 개체를 나타냅니다. `FtmBase`는 GIT (전역 인터페이스 테이블)를 만들고 마샬링 및 프록시 개체를 관리 하는 데 도움이 됩니다.
