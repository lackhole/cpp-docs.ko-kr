...
|[컴파일러 경고 (수준 1) C4812](compiler-warning-level-1-c4812.md)|사용 되지 않는 선언 스타일:를 사용 하십시오 '*입니다*' 대신|
...
| 컴파일러 경고 (수준 4) C4845 | \_\_declspec (아니요\_init\_all)'는 경우 무시 됩니다. ' / d1initall\[0\|1\|2\|3]' 명령줄에서 지정 되지 않았습니다 |
| 컴파일러 경고 (수준 4) C4846 | '*값*'에 대 한 올바른 인수가 아닙니다. ' / d1initall': 명령줄 플래그가 무시 |
...
|[컴파일러 경고 (수준 1, 오류) C4936](compiler-warning-c4936.md)|/clr 또는 /clr:pure를 지정하여 컴파일한 경우에만 이 __declspec를 사용할 수 있습니다.|
|[컴파일러 경고 (수준 4) C4937](compiler-warning-level-4-c4937.md)|'*text1*'및'*text2*'에 대 한 인수로 구분할 수 없습니다 는'*지시문*'|
...
|[컴파일러 경고 (수준 1) C4953](compiler-warning-level-1-c4953.md)|인라인 '*함수*' 프로필 데이터가 수집 된 이후 편집 되었습니다.|
|컴파일러 경고 C4954|'*함수*': 프로 파일링 되지 ((__int64 switch 식 포함)|
|컴파일러 경고 C4955|'*import2*': 가져오기 무시;에서 이미 가져온 '*import1*'|
|[컴파일러 경고 (수준 1, 오류) C4956](compiler-warning-c4956.md)|'*형식*':이 형식은 확인할 수 없습니다|
|[컴파일러 경고 (수준 1, 오류) C4957](compiler-warning-c4957.md)|'*캐스트*': 명시적 캐스트에서 '*cast_from*'to'*cast_to*'를 확인할 수 없습니다|
|[컴파일러 경고 (수준 1, 오류) C4958](compiler-warning-c4958.md)|'*작업*': 포인터 산술 연산은 확인할 수 없습니다|
|[컴파일러 경고 (수준 1, 오류) C4959](compiler-warning-c4959.md)|관리 되지 않는 형식을 정의할 수 없습니다 '*형식*' /clr: safe에서 해당 멤버에 액세스 하면 비안정형 코드가 생성 되므로|
|[컴파일러 경고 (수준 4) C4960](compiler-warning-level-4-c4960.md)|'*함수*' 너무 커서 프로 파일링 할 수 없습니다.|
...
| 컴파일러 경고 (수준 1) C5047 | 비표준 이용 \_ \_경우\_모듈에 있는 지원 되지 않습니다 |
| 컴파일러 경고 (수준 1) C5048 | 매크로의 사용 하 여 '*매크로 이름*' 명확 하지 않은 출력 될 수 있습니다 |
| 컴파일러 경고 (수준 1) C5049 | '*string*': 컴퓨터 종속 출력 발생할 전체 경로 포함 합니다. |
| 컴파일러 경고 (수준 1) C5050 | 모듈을 가져오는 동안 가능한 호환 되지 않는 환경 '*module_name*': *문제* |
| 컴파일러 경고 (수준 1) C5100 | \_\_VA\_ARGS\_ \_ variadic 매크로에서 사용 하도록 예약 되어 |
| 컴파일러 경고 (수준 1) C5101 | 동작은 정의 되지 않은 함수 형식 매크로 인수 목록의 전처리기 지시문 사용 |
| 컴파일러 경고 (수준 1) C5102 | 잘못 된 명령줄 매크로 정의 무시 합니다. '*값*' |
| 컴파일러 경고 (수준 1) C5103 | 붙여넣기 '*token1*'및'*token2*' 유효한 전처리 토큰을 얻지 |
| 컴파일러 경고 (수준 1) C5104 | 찾을 '*string1*#*string2*'매크로 대체 목록의 했습니까'*string1*"" #*string2*'? |
