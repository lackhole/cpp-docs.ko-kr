...
이 경고는 항상 오류로 실행 됩니다.  이 경고를 사용 하지 않도록 설정 하는 방법에 대 한 자세한 내용은 [warning](../../preprocessor/warning.md) pragma를 참조 하십시오.  또한이 경고는 compiland 당 한 번만 실행 됩니다. 따라서 Macintosh 형식으로 파일을 지정 하는 `#include` 지시문이 여러 개 있는 경우 C4335은 한 번만 실행 됩니다.
