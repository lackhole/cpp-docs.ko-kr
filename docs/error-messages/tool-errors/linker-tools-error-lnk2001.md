...
이 오류 때문일 수 있습니다 일치 하지 않는 경우 소스 코드에 모듈 정의 (.def) 파일입니다. 예를 들어 변수 이름을 지정 하면 `var1` 하나 C++ 원본 파일 및으로 액세스 하려고 `VAR1` 다른이 오류가 생성 됩니다. 이 문제를 해결 하려면 사용은 일관 되 게 철자를 입력 하 고 이름은 대/소문자입니다.
...
C 함수를 호출 하는 경우이 오류가 발생할 수 있습니다는 C++ 를 사용 하지 않고 프로그램을 `extern "C"` C 함수를 선언 합니다. 컴파일러는 C에 대 한 다른 내부 기호 명명 규칙을 사용 하 고 C++ 코드 및 해당은 내부 기호 이름에 대 한 사용자가 기호를 확인 하는 경우 링커는입니다. 이 문제를 해결 하려면 사용 하 여는 `extern "C"` 래퍼에 사용 되는 C 함수의 모든 선언에 C++ 하면 컴파일러가 해당 기호에 대 한 내부 C 명명 규칙을 사용 하는 코드입니다. 컴파일러 옵션 [/Tp](../../build/reference/tc-tp-tc-tp-specify-source-file-type.md) 하 고 [/Tc](../../build/reference/tc-tp-tc-tp-specify-source-file-type.md) 파일을 다음으로 컴파일하려면 컴파일러가 C++ 또는 C 파일 이름 확장명에 관계 없이 각각. 이러한 옵션에는 내부 함수 이름은 원하는 것과 다른 될 수 있습니다.

이 오류는 함수 또는 외부 링크가 없는 데이터를 참조 하려고 하 여 발생할 수 있습니다. C++, 인라인 함수 및 `const` 으로 명시적으로 지정 되지 않은 데이터는 내부 링크 `extern`합니다. 이 문제를 해결 하려면 사용 하 여 명시적 `extern` 정의 원본 파일 외부 기호에 대 한 선언을 참조 합니다.
...
이 오류를 반환 및 매개 변수 형식 또는 함수 정의 일치 하지 않는 호출 규칙을 사용 하는 함수 호출에서 발생할 수 있습니다. C++ 파일을 개체 [이름 데코레이션](../../error-messages/tool-errors/name-decoration.md) 호출 규칙, 클래스 또는 네임 스페이스 범위 및 함수의 반환 형식과 매개 변수 형식은 일치 하는 경우에 기호로 사용 되는 트 데코 레이 된 최종 함수 이름에 통합 함수 호출에 다른 개체 파일에서 확인 됩니다. 이 문제를 해결 하려면 선언, 정 및 모든 함수 호출에이 동일한 범위, 유형 및 호출 규칙을 사용 해야 합니다.

이 오류가 발생할 수 있습니다 C++ 클래스 정의에서 함수 프로토타입을 포함 하지만 하지 못했습니다. 코드 [구현을 포함](../../error-messages/tool-errors/missing-function-body-or-variable.md) 함수의 후 호출 합니다. 이 문제를 해결 하려면 클래스의 멤버를 선언 된 모든 호출에 대 한 정의 제공 해야 합니다.
...
- 이 문제는 (아마도 다른 위치로), 새 SDK가 설치 되어 있고 검색 순서를 새 위치를 가리키도록 업데이트 되지 않습니다 나타날 수 있습니다. 일반적으로 경로 추가 하는 새 SDK에 포함 및 라이브러리 디렉터리의 기본 시각적 개체 앞에 C++ 위치입니다. 또한 포함 된 경로 포함 하는 프로젝트는 유효 하지만 다른 위치에 설치 되어 있는 새 버전에서 추가 된 새 기능에 대 한 오래 된 이전 경로 가리키기도 합니다.
...
이 오류는.def 파일에 나열 된 내보내기는 찾을 수 없을 때 발생 합니다. 존재 하지 않는는 철자를 잘못 사용 하 여 또는 때문일 수 있습니다 C++ 데코 레이트 된 이름입니다. .Def 파일에 데코레이팅된 이름을 사용 하지 않습니다. 이 문제를 해결 하려면 불필요 한 내보내기를 제거 하 고 사용 하 여 `extern "C"` 내보낸된 기호에 대 한 선언 합니다.
