...
1. **오류: 비트맵 파일 리소스 파일 3.00 형식이 아닙니다.**
...
1. **기호 정보를 초기화할 수 없습니다.**
   ...
   시각적 개체에만 발생 C++입니다. 가능한 원인은 너무 많은 파일이 열려 있거나 열거나 시각적 개체에 필요한 데이터 파일에 쓸 수 없습니다 C++ 스크립트에서 기호를 가져오는 데 있습니다. Visual C++ 에서 지정한 디렉터리에 이러한 파일을 만들려고 시도 합니다 **TMP** 환경 변수 또는 지정 되지 않은 경우 현재 디렉터리입니다.

1. **기호 정보를 저장할 수 없습니다.**

   시각적 개체에만 발생 C++입니다. 가능한 원인은 너무 많은 파일이 열려 있거나 닫거나 시각적 개체에 필요한 데이터 파일에 쓸 수 없습니다 C++ 스크립트에서 기호를 가져오는 데 있습니다. Visual C++ 에서 지정한 디렉터리에서 이러한 파일을 사용 하려고 시도 합니다 **TMP** 환경 변수 또는 지정 되지 않은 경우 현재 디렉터리입니다.
...
1. **리소스가 너무 큽니다.**
   ...
   Windows 3.1의 경우 리소스는 대략 65000 바이트를 초과할 수 없습니다. 리소스는 경우 시각적 개체를 사용 하 여 컴파일할 수 없습니다 C++ 또는 명령줄 리소스 컴파일러. 커서, 아이콘, 비트맵 또는 기타 파일 기반 리소스에는 이 제한이 적용되지 않습니다.
