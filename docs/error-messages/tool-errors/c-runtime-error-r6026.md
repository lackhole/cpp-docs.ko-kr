...
> [!NOTE]
> 앱을 실행 하는 동안이 오류 메시지를 발생 하는 경우 앱 종료가 내부 메모리 문제가 있기 때문입니다. 이 오류의 원인은 있지만 일반적으로 메모리가 매우 부족 상태로 인해 발생 합니다. 것도 발생할 수 있습니다 앱의 버그로 인해 시각적 개체의 손상으로 C++ 를 사용 하는 라이브러리 또는 드라이버에서.
>
...
> - 사용 합니다 **앱 및 기능** 또는 **프로그램 및 기능** 페이지를 **제어판** 복구 하거나 Microsoft 시각적 개체의 모든 복사본을 다시 C++ 재배포 가능 패키지.
