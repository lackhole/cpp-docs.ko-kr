...
비동기 프로그래밍을 사용 하 여 Windows 런타임 앱 모델의 핵심 구성 요소 이므로 앱 사용자 입력에 응답을 유지할 수 있습니다. UI 스레드를 차단하지 않고 장기 실행 작업을 시작할 수 있으며 해당 작업의 결과를 나중에 받을 수 있습니다. 또한 작업을 취소하고 작업이 백그라운드에서 실행될 때 진행률 알림을 받을 수도 있습니다. 문서 [의 비동기 프로그래밍 C++ ](/windows/uwp/threading-async/asynchronous-programming-in-cpp-universal-windows-platform-apps) 사용할 수 있는 비동기 패턴의 개요를 제공 시각적 개체의 C++ UWP 앱을 만듭니다. 해당 문서 모두 사용 하 고 비동기 Windows 런타임 작업의 체인을 만드는 방법에 설명 합니다. 이 섹션에서는 ppltasks.h의 형식을 사용 하 여 다른 Windows 런타임 구성 요소에서 사용할 수 있는 비동기 작업을 생성 하는 방법을 설명 하 고 비동기 작업을 제어 하는 방법을 실행 됩니다. 또한 [비동기 프로그래밍 패턴 및 팁 hilo에서 (Windows 스토어 앱을 사용 하 여 C++ 및 XAML)](https://msdn.microsoft.com/library/windows/apps/jj160321.aspx) 인 hilo에서 비동기 작업을 구현 하려면 작업 클래스를 사용 하는 방법에 대해 알아보려면 사용 하 여 Windows 런타임 앱 C++ 및 XAML을 추가 합니다.
...
- [예제: 만들기는 C++ Windows 런타임 구성 요소](#example-component)

- [실행 스레드 제어](#exethread)

- [예제: 사용 하는 Windows 런타임 앱에서 실행 제어 C++ 및 XAML](#example-app)
...
Windows 런타임에서 특수 한 운영 체제 환경 에서만 실행 되는 UWP 앱을 만드는 데 사용할 수 있는 프로그래밍 인터페이스입니다. 이러한 앱 권한이 부여 된 함수, 데이터 형식 및 장치를 사용 하 고 Microsoft Store 배포 됩니다. Windows 런타임은 표현 합니다 *응용 프로그램 이진 인터페이스* (ABI). Windows 런타임 Api를 시각적 개체와 같은 프로그래밍 언어를 사용할 수 있도록 하는 기본 이진 계약은 ABI가 C++입니다.

Windows 런타임을 사용 하 여 다양 한 프로그래밍 언어의 유용한 기능을 사용할 수 있으며 하나의 앱에 결합할 수 있습니다. 예를 들어 JavaScript에서 UI를 만들고 C++ 구성 요소에서 계산이 많은 앱 논리를 수행할 수 있습니다. 이러한 계산이 많은 작업을 백그라운드에서 수행하는 기능은 UI 응답 성능을 유지하는 데 핵심적인 요소입니다. 때문에 합니다 `task` 클래스에 따라 다릅니다 C++를 다른 구성 요소에 비동기 작업에 전달할 Windows 런타임 인터페이스를 사용 해야 합니다 (있는 작성 될 수도 언어 이외의 C++). Windows 런타임 비동기 작업을 나타내는 데 사용할 수 있는 네 가지 인터페이스를 제공 합니다.
...
##  <a name="example-app"></a> 예: 사용 하는 Windows 런타임 앱에서 실행 제어 C++ 및 XAML
