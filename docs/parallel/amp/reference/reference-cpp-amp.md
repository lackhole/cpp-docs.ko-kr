...
`fast_math` 네임 스페이스 함수는 C99 규격이 아닙니다. 각 함수의 단정밀도 버전만 제공됩니다. 이러한 함수에서 DirectX 내장 함수를 사용하는데 `precise_math` 네임 스페이스의 해당 함수보다 빠르며 액셀러레이터에서 확장 배정밀도 지원이 필요하지는 않지만 정확도는 떨어집니다. C99 코드와의 소스 레벨 호환성을 위해 각 기능에는 두 가지 버전이 있습니다. 두 버전 모두 단정밀도 값을 가져와 반환합니다.
