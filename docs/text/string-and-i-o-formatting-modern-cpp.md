...
C++[\<iostream >](../standard-library/iostream.md) 클래스, 함수 및 연산자는 형식이 지정 된 문자열 i/o를 지원 합니다. 예를 들어 다음 코드는 정수를 16 진수로 출력 하도록 `cout`를 설정 하는 방법을 보여 줍니다. 먼저 현재 상태를 다시 설정 하기 위해 현재 상태를 저장 합니다. 즉, `cout`에 형식 상태가 전달 되 면 변경 될 때까지 해당 방법이 유지 됩니다. 한 줄의 코드에만 적용되는 것은 아닙니다.
