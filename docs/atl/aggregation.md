...
통합은 포함하는(외부) 개체가 생성된 프로세스의 일부로 포함된(내부) 개체를 만들고, 내구 개체의 인터페이스가 외부에 노출됨을 의미합니다. 개체는 자신이 통합 가능한지 여부를 결정할 수 있습니다. 그렇다면 통합이 제대로 동작하도록 특정 규칙을 수행해야 합니다.

기본적으로, 모든 `IUnknown` 메서드 호출은 포함이 되는 개체에 위임해야 합니다.
