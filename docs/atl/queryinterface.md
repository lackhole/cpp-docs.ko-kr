...
ms.locfileid: "69491999"
---
# <a name="queryinterface"></a>QueryInterface

개체가 인스턴스화되기 전에 정적으로 제공하는 기능을 표현할 수 있는 메커니즘이 있지만, 기본적으로 COM 메커니즘은 [QueryInterface](/windows/win32/api/unknwn/nf-unknwn-iunknown-queryinterface(q_))라는 `IUnknown` 메서드를 사용합니다.

모든 인터페이스는 `IUnknown`에서 파생되므로, 모든 인터페이스는 `QueryInterface` 구현이 있습니다. 구현에 관계없이 이 메서드는 호출자가 포인터 인터페이스의 IID를 사용하여 개체를 쿼리합니다. 개체가 해당 인터페이스를 지원하면 `QueryInterface`는 `AddRef`를 호출하면서 인터페이스에 대한 포인터를 검색합니다. 그렇지 않으면 E_NOINTERFACE 오류 코드를 반환 합니다.
...
[QueryInterface 개체 탐색](/windows/win32/com/queryinterface--navigating-in-an-object)
