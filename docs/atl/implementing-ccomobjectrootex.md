...
[CComObjectRootEx](../atl/reference/ccomobjectrootex-class.md) 반드시; 모든 ATL 개체의 인스턴스 하나가 있어야 합니다. `CComObjectRootEx` 하거나 [CComObjectRoot](../atl/reference/ccomobjectroot-class.md) 의 상속 합니다. `CComObjectRootEx`는 COM 맵 엔트리를 기반으로 하는 기본 `QueryInterface` 메커니즘을 제공합니다.
