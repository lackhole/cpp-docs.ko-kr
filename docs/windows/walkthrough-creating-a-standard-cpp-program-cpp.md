...
Visual Studio를 사용하여 표준 C++ 프로그램을 만들 수 있습니다. 프로젝트 생성, 프로젝트에 새 파일 추가, C++ 코드를 추가하여 파일 수정, 그리고 컴파일과 실행의 일련의 과정을 Visual Studio를 통해 단계별로 진행합니다.

C++ 프로그램을 직접 입력하거나 샘플 프로그램 중 하나를 이용할 수 있습니다. 이 연습에서는 샘플 프로그램은 콘솔 응용 프로그램입니다. 이 응용 프로그램에서는 C++ 표준 라이브러리의 컨테이너 `set`을 사용합니다.

> [!NOTE]
> 특정 버전을 C++ 언어 표준(예: C + + 14 또는 C + + 17)의 버전을 지정하여 컴파일하기 위하여 `/std:C++14` 또는 `/std:c++17` 컴파일러 옵션을 사용합니다. (Visual Studio 2017 이상입니다.)
...
### <a name="to-create-a-c-project-in-visual-studio-2019"></a>Visual Studio 2019에서 프로젝트 C++ 만들기
...
1. 필터링된 프로젝트 형식 목록에서 **콘솔 앱**을 선택한 후 **다음**을 선택합니다. 다음 페이지에서 프로젝트의 이름을 입력하 고 원하는 경우 프로젝트 위치를 지정 합니다.
...
### <a name="to-create-a-c-project-in-visual-studio-2017"></a>Visual Studio 2019에서 프로젝트 C++ 만들기
...
### <a name="to-create-a-c-project-in-visual-studio-2015"></a>Visual Studio 2019에서 프로젝트 C++ 만들기

1. 메뉴의 **파일**에서 **새로만들기**를 클릭한 다음 **프로젝트**를 선택하여 새 프로젝트를 만듭니다.

1. **Visual C++**의  프로젝트 형식 창에서 **Windows Desktop**을 클릭하고 **Windows 콘솔 응용 프로그램**을 클릭합니다.

1. **새 프로젝트**의 대화 상자에서 **설치 됨** > **템플릿** > **시각적 C++**, **Win32**를 선택합니다. 가운데 창에서 **Win32 콘솔 애플리케이션**을 선택합니다.
...
1. 다음과 같이 새 소스 파일을 프로젝트에 추가 합니다.
   ...
   **솔루션 탐색기**의 **소스 파일** 폴더에서 .cpp 파일이 보이면 파일을 Visual Studio 편집기에서 엽니다.

1. 편집기의 파일에서 C++ 표준 라이브러리를 사용하는 올바른 C++ 프로그램 샘플 프로그램 중 하나를 복사하여 파일에 붙여넣습니다.
