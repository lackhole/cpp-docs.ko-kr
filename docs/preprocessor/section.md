...
|**nopage**|섹션을 페이징할 수 없는 것으로 표시합니다. Win32 디바이스 드라이버에 유용합니다.|
|**nocache**|섹션을 캐시할 수 없는 것으로 표시합니다. Win32 디바이스 드라이버에 유용합니다.|
|**discard**|섹션을 삭제할 수 있는 것으로 표시합니다. Win32 디바이스 드라이버에 유용합니다.|
...
다음 예제에서 첫 번째 명령은 섹션과 해당 특성을 식별합니다. 정수 `j`는 `mysec` 로 선언되지 않았기 때문에 `__declspec(allocate)`에 삽입되지 않습니다. `j`는 데이터 섹션으로 이동합니다. 정수 `i`는 `mysec` 스토리지 클래스 특성의 결과로 `__declspec(allocate)`로 이동합니다.
