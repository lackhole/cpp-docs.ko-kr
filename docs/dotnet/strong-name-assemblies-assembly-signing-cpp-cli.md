...
시각적 개체를 사용 하는 경우 C++를 서명 하는 어셈블리에 대 한 CLR 특성을 관련 된 문제를 방지 하려면 어셈블리에 서명할 링커 옵션을 사용 합니다.
...
특성을 사용 하지 않는 이유는 키 이름을 파일 이름 기밀 정보를 포함 하는 경우 보안상 위험할 수 있는 어셈블리 메타 데이터에서 볼 수 있다는 사실을 포함 합니다. 또한 시각적 개체에서 사용 하는 빌드 프로세스 C++ 개발 환경에는 CLR 특성을 사용 하 여 어셈블리에 강력한 이름을 지정 하 고 다음 어셈블리에 mt.exe 같은 사후 처리 도구를 실행 하는 경우 어셈블리가 서명 되는 키를 무효화 됩니다.
