...
시작 시 초기화를 위하여 라이브러리가 의존하는 사용자 제공 함수에는 여러 가지 있습니다. 예를 들어, 전 세계의 연산자 오버 로드 하는 경우 C++ 와 같은 합니다 `new` 및 `delete` 에 포함 연산자, 사용자가 제공한 버전 어디서 든 사용 C++ 표준 라이브러리 초기화 및 소멸 합니다. 결과적으로, C++ 표준 라이브러리 및 사용자 제공 정적 이니셜라이저는 모든 사용자 제공 버전 이러한 연산자를 호출 합니다.
