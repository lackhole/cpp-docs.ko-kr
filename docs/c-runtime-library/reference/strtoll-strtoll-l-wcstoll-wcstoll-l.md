...
**strtoll** 은 표현이 오버플로를 발생 시키는 경우를 제외 하 고 *strtoll*에 표시 되는 값을 반환 합니다 .이 경우 **LLONG_MAX** 또는 **LLONG_MIN**을 반환 합니다. 변환을 수행할 수 없으면 이 함수는 0을 반환합니다. **wcstoll** 는와 유사 **값을 반환**합니다.

**LLONG_MAX** 및 **LLONG_MIN** 는 제한에 정의 되어 있습니다. 넣기.
...
로캘의 **LC_NUMERIC** 범주 설정은 *strsource*의 기 하 문자 인식 여부를 결정 합니다. 자세한 내용은 [setlocale, _wsetlocale](setlocale-wsetlocale.md)을 참조 하세요. **_L** 접미사가 없는 함수는 현재 로캘을 사용 합니다. **_strtoll_l** 및 **_wcstoll_l** 은 전달 된 로캘을 대신 사용 한다는 점을 제외 하 고 접미사가 없는 해당 함수와 동일 합니다. 자세한 내용은 [Locale](../../c-runtime-library/locale.md)을 참조하세요.
