...
**_chsize** 는 파일 크기가 성공적으로 변경 된 경우 값 0을 반환 합니다. 반환 값-1은 오류를 나타냅니다. 지정 된 파일이 읽기 전용 이거나 지정 된 파일이 액세스에 대해 잠긴 경우 **errno** 가 **eacces** 로 설정 되 고, 설명자가 잘못 된 경우 **ebadf** 로 설정 되 고, 장치에 남아 있는 공간이 없는 경우 **ENOSPC** 또는 *크기가* 0 보다 작은 경우 **EINVAL** 입니다.
