...
**strtomax** 는 표현이 오버플로를 발생 시키는 경우를 제외 하 고 문자열 *strsource*에 표시 되는 값을 반환 합니다 .이 경우 **INTMAX_MAX** 또는 **INTMAX_MIN**을 반환 하 고 **errno** 는 ERANGE로 설정 됩니다. 변환을 수행할 수 없으면 이 함수는 0을 반환합니다. **wcstoimax** 는와 유사를 **strtoimax**로 반환 합니다.
...
**Strtoimax** 함수는 *strsource* 를 **intmax_t**로 변환 합니다. **Strtoimax** 의 와이드 문자 버전은 **wcstoimax**입니다. *Strsource* 인수는 와이드 문자 문자열입니다. 그 외의 경우에는 이들 함수가 동일하게 동작합니다. 두 함수는 숫자의 일부분으로 인식할 수 없는 첫 번째 문자에서 문자열 *Strsource* 의 읽기를 중지 합니다. 이 문자는 종료 null 문자일 수도 있고 *base*보다 크거나 같은 첫 번째 숫자 문자일 수도 있습니다.

로캘의 **LC_NUMERIC** 범주 설정은 *strsource*의 기 하 문자 인식 여부를 결정 합니다. 자세한 내용은 [setlocale, _wsetlocale](setlocale-wsetlocale.md)을 참조 하세요. **_L** 접미사가 없는 함수는 현재 로캘을 사용 합니다. **_strtoimax_l** 및 **_wcstoimax_l** 은 전달 된 로캘을 대신 사용 한다는 점을 제외 하 고 **_l** 접미사가 없는 해당 함수와 동일 합니다. 자세한 내용은 [Locale](../../c-runtime-library/locale.md)을 참조하세요.
