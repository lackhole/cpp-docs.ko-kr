...
**strtoull** 는 변환 된 값 (있는 경우)을 반환 하거나 오버플로가 발생 하면 **ULLONG_MAX** 을 반환 합니다. 변환을 수행할 수 없는 경우 **strtoull** 는 0을 반환 합니다. **wcstoull** 는와 유사 값을 **strtoull**로 반환 합니다. 두 함수 모두 오버플로 또는 언더플로가 발생 하는 경우 **errno** 는 **ERANGE** 로 설정 됩니다.
...
**strtoull** 는 숫자의 일부분으로 인식할 수 없는 첫 번째 문자에서 문자열 *strsource* 읽기를 중지 합니다. 이 문자는 종료 null 문자일 수도 있고 *base*보다 크거나 같은 첫 번째 숫자 문자일 수도 있습니다. 로캘의 **LC_NUMERIC** 범주 설정에 따라 *strsource*의 기 개수 문자 인식이 결정 됩니다. 자세한 내용은 [setlocale, _wsetlocale](setlocale-wsetlocale.md)을 참조 하세요. **strtoull** 및 **wcstoull** 는 현재 로캘을 사용 합니다. **_strtoull_l** 및 **_wcstoull_l** 은 전달 된 로캘을 대신 사용 하지만 그렇지 않은 경우 동일 합니다. 자세한 내용은 [Locale](../../c-runtime-library/locale.md)을 참조하세요.
