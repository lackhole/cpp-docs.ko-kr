...
Microsoft C++ 컴파일러(MSVC)를 사용하여 64 비트 Windows 운영 체제에서 실행되는 응용 프로그램을 만들경우 다음 문제에 유의 하세요.

- `int`와 `long`은 64비트 Windows 운영 체제상에서 32비트 값입니다. 프로그램을 64비트 플랫폼 용으로 컴파일하려는 계획이 있다면 포인터를 32비트 변수에 할당하지 않도록 주의해야 합니다. 64비트 플랫폼에서 포인터는 64비트이며, 이를 32비트 변수에 할당하면 포인터 값이 잘립니다.

- `size_t`, `time_t` 및 `ptrdiff_t`는 64 비트 Windows 운영 체제상에서 64 비트 값입니다.

- `time_t`는 Visual Studio 2005 및 이전으로 32 비트 Windows 운영 체제에서는 32 비트 값입니다. `time_t`는 이제 기본적으로 64비트 정수입니다. 자세한 내용은 [시간 관리](../c-runtime-library/time-management.md)를 참조합니다.
...
- %p(16진수 포인터 형식)는 64비트 Windows 운영 체제에서 제대로 출력됩니다.
