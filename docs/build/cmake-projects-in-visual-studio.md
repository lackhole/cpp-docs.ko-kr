...
> [!NOTE]
> 지난 몇 번의 릴리스에서 통해 Visual Studio를 사용 하 여 CMake는 점점 더 통합 되 되어 있습니다. 사용 중인 버전에 대 한 올바른 정보를 보려면이 페이지의 왼쪽 위에 있는 버전 선택기를 올바르게 설정 되어 있는지 확인 합니다. 
...
- Visual Studio 추가 **CMake** 항목을 **프로젝트** 보기 및 CMake 스크립트 편집에 대 한 명령 사용 하 여 메뉴.
...
라는 파일을 사용 하 여 visual Studio **CMakeSettings.json** Cmake.exe 위한 명령줄 옵션 또는 환경 변수를 저장 합니다. **CMakeSettings.json** 또한 정의 여러 CMake를 저장 하 여 빌드 구성 및 편리 하 게 ide에서 사이 전환 합니다. Visual Studio 2019에는 **CMake 설정 편집기** 설정을 편집 하는 편리한 방법을 제공 합니다. 참조 [CMake 사용자 지정 설정](customize-cmake-settings.md) 자세한 내용은 합니다.
...
사용자 지정할 수 있습니다 빌드 구성, 환경 변수, 명령줄 인수 및 기타 설정을 사용 하 여 CMakeLists.txt 파일을 수정 하지 않고 합니다 **CMake 설정 편집기**합니다. 자세한 내용은 [CMake 설정 사용자 지정](customize-cmake-settings.md)을 참조하세요.
...
설치한 후 [vcpkg](vcpkg.md), CMake 프로젝트를 Visual Studio에서 열릴 vcpkg 도구 체인 파일을 자동으로 통합 됩니다. 즉, 추가 구성 없이 CMake 프로젝트를 사용 하 여 vcpkg를 사용 해야 합니다. 이 지원은 로컬 vcpkg 설치와 대상으로 하는 원격 컴퓨터에 vcpkg 설치에 적합 합니다. CMake 설정 구성에 다른 도구 체인을 지정 하는 경우이 동작은 자동으로 비활성화 됩니다.

## <a name="just-my-code-for-cmake-projects"></a>CMake 프로젝트에 대 한 내 코드만

를 빌드할 때 Windows에 대 한 MSVC 컴파일러를 사용 하 여 CMake 프로젝트를 이제 내 코드만 디버깅 컴파일러 및 링커 옵션은 Visual Studio에서 설정 하는 경우 지원 합니다. 로 이동 하 여 설정을 변경 하려면 **도구가** > **옵션** > **디버깅** > **일반**합니다.

## <a name="customize-configuration-feedback"></a>구성 피드백을 사용자 지정

기본적으로 대부분의 구성 메시지가 없으면 다음 오류가 표시 되지 않습니다. 이 기능을 사용 하 여 모든 메시지를 볼 수 있습니다 **도구가** > **옵션** > **CMake**합니다.
