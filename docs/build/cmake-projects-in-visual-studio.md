...
**C++ Cmake 도구** 구성 요소는 [폴더 열기](https://docs.microsoft.com/en-us/cpp/build/open-folder-projects-cpp?view=vs-2019) 기능을 사용 하 여 IntelliSense 및 검색을 위해 직접 cmakelists .txt와 같은 cmakeproject 파일을 사용 합니다. Ninja 및 Visual Studio 생성기 모두 지원됩니다. Visual Studio 생성기를 사용하는 경우 임시 프로젝트 파일이 생성되어 msbuild.exe에 전달되지만 IntelliSense 또는 검색을 위해 로드되지는 않습니다. 기존 CMake 캐시를 가져올 수도 있습니다.
...
- Visual Studio는 cmake를 실행 하 고 기본 *구성*(x64 디버그)에 대해 ctocache를 생성 합니다. CMake 명령줄이 CMake의 추가 출력과 함께 **출력 창**에 표시됩니다.
...
Visual Studio에서는 **Cmakesettings. json** 이라는 파일을 사용 하 여 cmage.exe에 대 한 환경 변수 또는 명령줄 옵션을 저장 합니다. **Cmakesettings. json** 을 사용 하면 여러 cbuild 빌드 구성을 정의 하 고 저장 하며 IDE에서이를 편리 하 게 전환할 수 있습니다. Visual Studio 2019에서는 **Cmake 설정 편집기** 를 사용 하 여 설정을 편리 하 게 편집할 수 있습니다. 자세한 내용은 [cmake 설정 사용자 지정](customize-cmake-settings.md) 을 참조 하세요.
...
기존 CMake 캐시를 열 때 Visual Studio는 캐시 및 빌드 트리를 관리 하려고 시도 하지 않습니다. 이를 통해 사용자 지정 또는 기본 설정 도구는 cmake에서 프로젝트를 구성 하는 방법을 완벽 하 게 제어할 수 있습니다. > 파일을 통해 Visual Studio에서 기존 캐시를 열 수 있습니다. **> cmake를 열고** 기존 cmakecache.txt로 이동 합니다. 또는 Visual Studio에서 프로젝트를 이미 연 경우 새 구성을 추가 하는 것과 같은 방법으로 기존 캐시를 추가할 수 있습니다. 자세한 내용은 [Visual Studio에서 기존 캐시 열기](https://devblogs.microsoft.com/cppblog/open-existing-cmake-caches-in-visual-studio/)에 대 한 블로그 게시물을 참조 하세요.
...
**CMakeLists 편집기**를 사용 하 여 CMakeLists .txt 파일을 수정 하지 않고 빌드 구성, 환경 변수, 명령줄 인수 및 기타 설정을 사용자 지정할 수 있습니다. 자세한 내용은 [CMake 설정 사용자 지정](customize-cmake-settings.md)을 참조하세요.
...
기본적으로 오류가 발생 하지 않는 한 대부분의 구성 메시지는 표시 되지 않습니다. **도구** > **옵션** > **csa**에서이 기능을 사용 하도록 설정 하 여 모든 메시지를 볼 수 있습니다.
