...
기본적으로 DUMPBIN은 대상 개체 파일, 라이브러리 또는 실행 파일에 대 한 PDB 파일을 로드 하려고 합니다. DUMPBIN은이 정보를 사용 하 여 주소를 기호 이름에 일치 시킵니다. PDB 파일이 크거나 원격 서버에서 로드 되어야 하는 경우 프로세스에 시간이 오래 걸릴 수 있습니다. **/Nopdb** 옵션은이 단계를 건너뛰도록 DUMPBIN에 지시 합니다. 개체 파일, 라이브러리 또는 실행 파일에서 사용할 수 있는 주소 및 기호 정보만 인쇄 합니다.
...
1. **추가 옵션** 상자에 **/nopdb** 옵션을 추가 합니다. **확인** 또는 **적용** 을 선택 하 여 변경 내용을 저장 합니다.
