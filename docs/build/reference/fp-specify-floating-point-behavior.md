...
작업 및 부동 소수점 문의 식 순서 부동 소수점 코드를 받지 않습니다 하는 경우 (중요 하지 않은 경우에 예를 들어 있는지 여부를 `a * b + a * c` 로 계산 됩니다 `(b + c) * a` 또는 `2 * a` 으로 `a + a`)를 고려 합니다 [/fp:fast](#fast) 옵션을 보다 빠르고 보다 효율적인 코드를 생성할 수 있습니다. 코드 모두 작업 및 식 순서에 따라 및 액세스 또는 변경 (예: 반올림 모드를 변경 하거나 부동 소수점 예외를 트래핑 하) 부동 소수점 환경을 경우 사용하여 [/fp: strict](#strict)합니다.
...
`/fp:strict` 계산 보다 비용이 더 높은 `/fp:precise` 하므로 컴파일러 예외를 트래핑 하 고 액세스 하거나 런타임에 부동 소수점 환경을 수정 프로그램에 추가 지침을 삽입 해야 합니다. 코드는이 기능을 사용 하지 않습니다 하지만 소스 코드 순서 지정 및 반올림 필요, 특수 값을 사용 하는 경우 사용하여 `/fp:precise`입니다. 그렇지 않은 경우 사용 하는 것이 좋습니다 `/fp:fast`, 빠르고 작은 코드를 생성할 수 있습니다.

#### <a name="fast"></a>빠른

`/fp:fast` 옵션을 사용 하면 컴파일러를 다시 정렬, 결합, 아니면 속도 및 공간에 대 한 부동 소수점 코드를 최적화 하는 부동 소수점 작업을 간소화 합니다. 컴파일러는 대입문에서 반올림을 생략할 수, 대입문 또는 함수 호출 합니다. 작업 순서를 변경 또는 이러한 변형을 만들 띄는 다른 반올림 동작을 수행 하는 경우에 연관 및 분배 법칙을 사용하여 예를 들어, 대 수 변형을 수행할 수 있습니다. 이러한 향상 된 최적화 때문에 일부 부동 소수점 계산의 결과에서 다른 생성 된 달라질 `/fp` 옵션입니다. 특수 값 (NaN, + infinity,-infinity,-0.0) 전파할 수 없습니다 또는 IEEE-754 표준에 따라 엄격 하 게 동작 합니다. 부동 소수점 축약에서 생성 될 수 있습니다 `/fp:fast`합니다. 컴파일러에서 기본 아키텍처를 기준으로 여전히 바인딩되어 `/fp:fast`, 추가 최적화를 사용하여 사용할 수는 [/arch](arch-minimum-cpu-architecture.md) 옵션입니다.
...
`/fp:fast` 순서 지정 및 부동 소수점 식의 반올림 엄격 하 게 소스 코드가 필요 없는 및 NaN 같은 특수 한 값을 처리 하기 위한 표준 규칙에 의존 하지 마십시오는 프로그램에 대 한 것입니다. 부동 소수점 코드 순서 지정 및 반올림, 소스 코드를 유지 해야 하거나 특수 한 값을 사용하여의 표준 동작에 의존 하는 경우 [/fp: 정확한](#precise)합니다. 코드에 액세스 또는 부동 소수점 반올림 모드를 변경할 환경을 수정 하는 경우 부동 소수점 예외를 마스크 해제 또는 부동 소수점 상태를 확인, 사용하여 [/fp: strict](#strict)합니다.
...
### <a name="using-compiler-directives-to-control-floating-point-behavior"></a>컴파일러 지시문을 사용하여 부동 소수점 동작을 제어 하려면
...
컴파일러가 부동 소수점 환경의에서 기본값을 가정 하므로 `/fp:fast` 하 고 `/fp:precise` 에 대 한 호출을 무시 하는 것이 없으면 `_controlfp_s`합니다. 예를 들어, 둘 다를 사용하여 컴파일하면 `/O2` 고 `/fp:precise` x86 아키텍처 경계 계산 되지 않는 및 샘플 프로그램 출력:
...
둘 다를 사용하여 컴파일하면 `/O2` 및 `/fp:strict` x86 아키텍처 샘플 프로그램 출력:
...
사용하여 컴파일하면 `/O2` `/fp:precise` 또는 `/O2` `/fp:strict` x86 아키텍처, 출력은 IEEE-754 사양과 일치:
...
사용하여 컴파일하면 `/O2` `/fp:fast` x86 아키텍처 출력 IEEE-754와 일치 하지 않습니다.:
...
사용하여 컴파일하면 `/O2` `/fp:precise` 하거나 `/O2` `/fp:strict`, 명시적 형식 캐스트 typecast를 및 x64에 대 한 생성된 된 코드에서 함수 반환 점에서 삽입 되는 것을 볼 수 있습니다 아키텍처:
