...
컴파일러가 프로그램 이미지와 실행하는 동안 메모리에 동일한 문자열 복사본을 사용하도록 만듭니다. 이 최적화는 *문자열 풀링*이라고 불리며, 보다 작은 프로그램을 만들 수 있습니다.
...
**/GF**는 문자열을 읽기 전용으로 간주합니다. **/GF**가 적용된 상황에서 문자열을 수정 하려고하면 응용 프로그램 오류가 발생 합니다.

문자열 풀링은 다중 버퍼에 대한 다중 포인터를 단일 버퍼에 대해 다중 포인터로 간주합니다. 다음 코드에서 `s`와 `t`는 메모리상의 같은 문자열로 초기화됩니다. 문자열 풀링을 사용하면 동일한 메모리를 가리킬 수 있습니다.
