...
**queue**<br/>
오류 보고서를 큐에 넣습니다. 관리자 권한으로 로그인하면 마지막 로그인한 이후 발생한 모든 오류를 보고할 수 있도록 창이 표시됩니다(3일에 두 번 이상 오류에 대한 보고서를 보내라는 메시지를 표시하지 않습니다). **queue**는 명령 프롬프트에서 응용 프로그램을 컴파일할 때 기본값입니다.

**send**<br/>
Windows 오류 보고 시스템 설정으로 보고를 사용하는 경우 Microsoft에 내부 컴파일러 오류 보고서를 자동으로 보냅니다.

## <a name="remarks"></a>설명

내부 컴파일러 오류(ICE)는 컴파일러에서 소스 코드 파일을 처리할 수 없을 때 발생합니다. ICE가 발생하면 컴파일러에서 출력 파일 또는 코드를 수정하는 데 사용할 수 있는 유용한 진단을 생성하지 않습니다.

이전 릴리스에서는 ICE가 있을 때 Microsoft 기술 지원 서비스를 호출하는 것을 권장했습니다. **/errorReport**를 사용하면 Microsoft로 직접 ICE 정보를 보낼 수 있습니다. 오류 보고서는 향후 컴파일러 제품을 개선하는 데 도움이 됩니다.
