...
데코 레이트 된 명명 규칙, Visual Studio의 다양 한 버전에서 변경 되었고 대상 아키텍처에 다른 수도 있습니다. Visual Studio에서 C를 사용 하 여 만든 소스 파일과 올바르게 연결 하 고 C++ Dll 및 라이브러리를 동일한 컴파일러 도구 집합, 플래그 및 대상 아키텍처를 사용 하 여 컴파일해야 합니다. 

> [!NOTE]
> Visual Studio 2017 또는 Visual Studio 2019로 작성 된 응용 프로그램에서 Visual Studio 2015를 사용 하 여 빌드된 라이브러리를 사용할 수 있습니다.
...
이름 장식은 다른 프로그래밍 언어 또는 다른 컴파일러를 사용하여 작성된 코드에 연결할 경우에도 중요합니다. 컴파일러에 따라 다른 이름 장식 규칙이 사용됩니다. 실행 파일이 다른 언어로 작성된 코드에 연결되면 내보낸 이름/가져온 이름이 호출 규칙과 일치하도록 주의해야 합니다. 어셈블리 언어 코드가 MSVC를 사용 하 여 작성 된 소스 코드에 연결할 MSVC 데코 레이트 된 이름 및 호출 규칙을 사용 해야 합니다.
