...
**/FA** 컴파일러 옵션은 컴파일할 때 C 또는 C++ 소스 파일에 해당하는 각 변환 단위에 대한 어셈블러 목록 파일을 생성합니다. 기본적으로 ANSI로 인코딩되는 목록 파일에는 어셈블러만 포함되어 있습니다. **/FA**에 대한 선택적 인수 **c**, **s** 및 **u** 인수는 어셈블러 목록과 함께 기계 코드 또는 소스 코드를 출력할지 여부와 목록이 UTF-8로 인코딩되는지 여부를 제어합니다.
