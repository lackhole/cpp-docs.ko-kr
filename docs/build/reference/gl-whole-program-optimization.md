...
전체 프로그램 최적화는 컴파일러가 모든 모듈에 대한 정보를 사용하여 프로그램의 최적화를 수행할 수 있게 합니다. 전체 프로그램 최적화가 없으면 컴파일 되는 모듈 별로 수행됩니다.
...
- 전역 데이터에 대한 수정 내용을 추적하여 로드 및 저장 횟수를 줄여 더 효율적으로 작업이 수행 됩니다.

- 포인터 역 참조로 수정된 가능한 항목 집합을 추적하여 로드 및 저장 횟수를 줄여 더 효율적으로 작업이 수행 됩니다.
...
**/GL**로 생성된 .obj 파일과 미리 컴파일 된 헤더 파일은 .lib 파일이 **/GL** .obj 파일을 생성한 동일한 컴퓨터에 연결되지 않으면 .lib 파일을 작성하는데 사용하지 않아야 합니다. .obj 파일의 미리 컴파일 된 헤더 파일의 정보는 링크 할 때 필요합니다.

사용 가능한 최적화 및 전체 프로그램 최적화의 제한 사항에 대한 자세한 내용은 [/LTCG](ltcg-link-time-code-generation.md)를 참조합니다.  또한 **/GL**은 프로필 기반된 최적화를 사용할 수 있게합니다. /LTCG를 참조 하십시오.  프로필 기반 최적화를 반영한 컴파일 시 프로필 기반 최적화되는 함수 순서를 지정하려면 [/Gy](gy-enable-function-level-linking.md) 또는 컴파일러 옵션 /Gy를 암시하는 컴파일을 해야 합니다.

### <a name="to-set-this-linker-option-in-the-visual-studio-development-environment"></a>Visual Studio 개발 환경에서 이 링커 옵션을 설정하려면

1. **/GL**을 개발환경에서 지정하는 방법은 [/LTCG (링크 타임 코드 생성)](ltcg-link-time-code-generation.md)를 참조하세요.
