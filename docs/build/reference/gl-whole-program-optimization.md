...
/GL로 생성된 .obj 파일은 [EDITBIN](editbin-reference.md) 및 [DUMPBIN](dumpbin-reference.md) 같은 링커 유틸리티에서 사용할 수 없습니다.
...
현재 버전에서 /GL로 생성된 파일 형식은 이후 버전의 Visual C++에서 읽을 수 없습니다. 현재 및 미래에 사용자가 사용할 것으로 예상되는 모든 버전의 Visual C++에 대한 .lib 파일의 복사본을 제공하려는 경우가 아니면 /GL로 생성된 .obj 파일로 구성된 .lib 파일을 제공하지 않아야 합니다.
