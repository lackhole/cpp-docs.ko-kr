...
컴파일러는 처음 컴파일 중 소스 파일과 .idb 파일의 클래스 정의 간의 종속성 정보를 저장합니다. (종속성 정보는 어떤 소스 파일이 어떤 클래스 정의에 종속되어 있고 어떤 .h 파일에 정의가 있는지 알려줍니다.) 수정된 .h 파일이 포함된 경우에 어떤 소스 파일을 컴파일 해야 하는지 여부는 .idb 파일에 저장된 정보를 사용하는 후속 컴파일에서 이루어집니다.

> [!NOTE]
> 최소 다시 빌드는 포함 파일간에 변경되지 않는 클래스 정의를 사용합니다. .idb 파일의 종속성 정보가 전체 프로젝트에 대해 생성되기 때문에 클래스 정의는 프로젝트에 대해 전역적이어야 합니다 (지정된 클래스의 정의는 하나만 있어야합니다.) 프로젝트에 클래스 정의가 둘 이상 있는 경우 최소 다시 빌드를 비활성화 하십시오.
...
1. 선택된 **구성 속성** > **C/C++** > **코드 생성** 속성 페이지.

1. 수정된 **최소 다시 빌드** 속성입니다.
