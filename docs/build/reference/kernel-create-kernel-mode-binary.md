...
**/kernel**<br/>
현재 프로젝트의 코드를 컴파일 및 연결 집합을 사용 하 여 C++ 커널 모드에서 실행 되는 코드에만 적용 되는 언어 규칙입니다.

**/kernel-**<br/>
현재 프로젝트의 코드를 컴파일하여 사용 하지 않고 연결 된 C++ 커널 모드에서 실행 되는 코드에만 적용 되는 언어 규칙입니다.

## <a name="remarks"></a>설명

방법이 없는 `#pragma` 이 옵션을 제어 하에 해당 합니다.

지정 된 **/kernel** 옵션을 사용 하면 컴파일러 및 링커에서의 커널 모드에서 허용 되는 언어 기능을 조정 하는 데 충분 한 능력과에 고유한 런타임 안정성 문제를 방지 하려면 있는지 확인 커널 모드 C++입니다. 이렇게 사용을 금지 하 여 C++ 에 대 한 경고를 제공 하 고 커널 모드에서 중단 된 언어 기능 C++ 언어 기능을 잠재적으로 중단 되지만 비활성화할 수 없습니다.

합니다 **/kernel** 옵션 컴파일러와 링커 둘 다 빌드 단계를 적용 하 고 프로젝트 수준에서 설정 됩니다. 전달 된 **/kernel** 에 알리기 위해 컴파일러는 결과 이진 연결 후에 로드 되어야 할 Windows 커널이 전환 합니다. 컴파일러의 스펙트럼 좁혀집니다 C++ 커널와 호환 되는 하위 집합에 대 한 언어 기능입니다.
