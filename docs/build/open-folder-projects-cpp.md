...
**정의로 이동** 이 제대로 작동 하는 것과 같은 IntelliSense 및 검색 동작의 경우, Visual Studio에서 사용 중인 컴파일러, 시스템 헤더가 있는 위치 및 추가 포함 파일이 있는 위치를 알아야 합니다. 열려는 폴더 (작업 영역 폴더)입니다. 구성을 지정 하려면 기본 도구 모음의 드롭다운 목록에서 구성 **관리** 를 선택할 수 있습니다.

![구성 관리 드롭다운](media/manage-configurations-dropdown.png)

현재 Visual Studio는 Microsoft C++ 컴파일러에 대 한 네 가지 기본 구성을 제공 합니다.

![기본 구성](media/default-configurations.png)

예를 들어, **x64-Debug**를 선택 하는 경우 Visual Studio는 루트 프로젝트 폴더에 *cppproperties.json* 라는 파일을 만들고 다음과 같이 채웁니다.
...
Microsoft C++이외의 컴파일러를 사용 하는 경우 *cppproperties.json*에서 사용자 지정 구성 및 환경을 만들어야 합니다. 다음 예제에서는 MSYS2 설치에서 GCC를 사용 하기 위한 단일 사용자 지정 구성을 사용 하는 전체 *cppproperties.json* 파일을 보여 줍니다.
