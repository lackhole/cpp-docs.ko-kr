...
Windows on ARM은 SEH ( [구조적 예외 처리](/windows/win32/debug/structured-exception-handling) ) 중에 *해제 코드* 를 사용 하 여 스택 해제를 제어 합니다. 해제 코드는 실행 가능 이미지의 .xdata 섹션에 저장되는 바이트 시퀀스로, 함수 프롤로그 및 에필로그 코드의 작동을 요약하여 설명하므로 호출자 스택 프레임 해제를 준비하기 위해 적용된 함수 프롤로그를 실행 취소할 수 있습니다.
...
### <a name="example-1-leaf-function-no-locals"></a>예제 1: 리프 함수, 지역 없음
...
### <a name="example-2-nested-function-with-local-allocation"></a>예제 2: 로컬 할당이 포함 된 중첩 함수
...
### <a name="example-3-nested-variadic-function"></a>예제 3: Nested Variadic 함수
...
### <a name="example-4-function-with-multiple-epilogues"></a>예제 4: 여러 에필로그 함수
...
### <a name="example-5-function-with-dynamic-stack-and-inner-epilogue"></a>예 5: Dynamic Stack 및 Inner 에필로그 함수
...
- 단어 1: 에필로그 범위에서 오프셋 0xC6 (= 0x18C/2), 해제 코드 인덱스를 0x00에 시작 하 고 조건이 0x0E (항상) 인 경우
...
### <a name="example-6-function-with-exception-handler"></a>예 6: 예외 처리기를 사용 하는 함수
...
### <a name="example-7-funclet"></a>예 7: Funclet
