...
C를 미리 컴파일할 수 있습니다 하 고 C++ 프로그램입니다. C++ 를 프로그래밍 하는 것이 헤더 파일로 클래스 인터페이스 정보를 구분 하는 것이 있습니다. 나중에 클래스를 사용 하는 프로그램에 이들 헤더 파일을 포함할 수 있습니다. 이러한 헤더를 미리 컴파일하면, 프로그램을 컴파일하는 데 걸리는 시간을 줄일 수 있습니다.
...
> [!NOTE]
>  C에만 사용 하기 위한 미리 컴파일된 헤더 시설 및 C++ 소스 파일입니다.
...
소프트웨어 프로젝트의 코드 베이스는 일반적으로 여러 C에 포함 하거나 C++ 소스 파일, 개체 파일, 라이브러리 및 헤더 파일입니다. 일반적으로 메이크파일을 이러한 요소의 결합 한 실행 파일을 조정합니다. 다음 그림은 미리 컴파일된 헤더 파일을 사용 하는 메이크파일의 구조를 보여줍니다. NMAKE 매크로 이름 및 파일 이름을이 다이어그램에 있는 예제 코드와 일치 되 [PCH 용 샘플 메이크파일](#sample-makefile-for-pch) 하 고 [PCH에 대 한 예제 코드](#example-code-for-pch)합니다.
...
이러한 매크로를 나열 하는 파일 헤더 파일이 나 C 수 또는 C++ 소스 파일입니다. (모두 C를 사용 하 여 단일 PCH 파일을 사용할 수 없습니다 하 고 C++ 모듈.) 사용할 수 있는 참고 합니다 **hdrstop** 미리 컴파일 경계 파일 내에서 특정 시점을 중지 하는 매크로입니다. 참조 [hdrstop](../preprocessor/hdrstop.md) 자세한 내용은 합니다.
